...
# ---------------------------------------------------------------------------
# In-memory database schema. This mirrors bot.py's fresh-install CREATE
# TABLE statements exactly, so helper.helpers can be tested against a real
# sqlite backend without ever touching the project's actual database file.
# ---------------------------------------------------------------------------
...
DUELS_SCHEMA = (
    ...
    "challengerId, challengerName, targetId, targetName, amount, state, createdAt INTEGER)"
)
LEADERBOARDS_SCHEMA = (
...
TEAM_INVITES_SCHEMA = (
    ...
    "messageId, teamId, teamName, inviterId, targetId, targetName, createdAt INTEGER)"
)
TEAM_TRANSFERS_SCHEMA = (
    "CREATE TABLE team_transfers(id INTEGER PRIMARY KEY AUTOINCREMENT, guildId, channelId, "
    "messageId, teamId, teamName, fromCaptainId, fromCaptainName, toCaptainId, toCaptainName, "
    "createdAt INTEGER)"
)
...
def make_db():
    ...
    cursor.execute(TEAM_TRANSFERS_SCHEMA)
...
# .with_format() that swaps the extension. That matches the real thing
# since statsHelper's with_format("png") call needs to work whether the
...
class FakeAsset:
    def __init__(self, url):
        ...
        # _swapStatsForTradingCard) actually opens this with PIL, the same
...
class FakeMember:
    def __init__(self, name, id=None, bot=False, manage_guild=True):
        ...
        # different URL shape than FakeUser's below. That way a test can
        # tell which one actually ended up on screen (see the avatar-toggle
...
class FakeChannel:
    def __init__(self, name, id=None, members=None, kind="voice", guild=None):
        ...
        self.kind = kind
        self.guild = guild
        # Each call gets a fresh FakeMessage, not one shared return_value.
        # Real Discord messages are distinct objects, and printEmbed's
        # callers now rely on team1_message/team2_message being genuinely
        # different messages (see _finalizeRoster, which reacts to team2's
        # only). channel=self matches real discord.Message, which always
        # carries the channel it was sent/fetched in, so _finalizeRoster
        # can read it straight off the message instead of needing it passed
        # separately. Messages are tracked in _sent_messages by id so
        # fetch_message (see _updateDraftEmbeds) resolves back to the SAME
        # object a prior send() returned, rather than a fresh unrelated
        # one. A test checking edit.call_count across several picks needs
        # that same object each time, the same way a real Discord fetch
        # would give it.
        self._sent_messages = {}

        async def _send(*a, **k):
            message = FakeMessage(channel=self)
            self._sent_messages[message.id] = message
            return message

        async def _fetch_message(message_id):
            # Cached lazily, not just defaulted, so a second fetch of an id
            # that was never actually send()'d through this channel (say, a
            # test that seeds a roster_team*_message_id by hand) still
            # resolves back to the same object every time. That matches a
            # real Discord fetch's own stable identity.
            if message_id not in self._sent_messages:
                self._sent_messages[message_id] = FakeMessage(id=message_id, channel=self)
...
class FakeGuild:
    def __init__(self, id=GUILD_ID, name="Test Guild", channels=None, members=None, system_channel=None):
        ...
        # The bot's own member in this guild (discord.py's Guild.me). This is
        # what permissions_for() checks are actually made against, but it's
        # just a placeholder identity: no test currently varies it, only
...
class FakeClient:
    def __init__(self, channels=(), guilds=(), users=()):
        ...
        # A cache miss (get_user returns None) is the common case in
        # tests. fetch_user still resolves to a real (if auto-generated)
        # FakeUser rather than erroring, the same way the real API always
        # eventually resolves a valid id.
...
class TeamTests(unittest.TestCase):
    ...
    def test_wins_and_losses_are_not_part_of_serialization(self):
        # wins/losses are pure display state. helper._hydrateTeamGameRecord
        # sets them from team_game_stats after every load. They never
        # round-trip through serializeTeam/deserializeTeam itself, so a
        # freshly-deserialized Team (bypassing that hydration step, as this
        # test does) always starts at the plain 0 default.
        team = Team()
        team.set_id(1)
        team.set_name("Team 1")
        team.wins = 7
...
class FinalizeRosterTests(HelperTestCase):
    ...
    async def test_always_attaches_a_roster_action_view_without_role_buttons(self):
    ...
    async def test_includes_role_buttons_when_both_teams_are_exactly_five(self):
        channel = FakeChannel("game-chat")
        team1_message = FakeMessage()
        team2_message = FakeMessage()
        team2_message.channel = channel
        ...
        self.assertIn(view.balanceRoles, view.children)
    ...
    # them is actually clicked.
    async def test_includes_role_buttons_for_a_five_a_side_roster_that_never_used_roles(self):
        channel = FakeChannel("game-chat")
        team1_message = FakeMessage()
        team2_message = FakeMessage()
        team2_message.channel = channel
    ...
    async def test_skips_role_buttons_when_not_five_a_side(self):
        channel = FakeChannel("game-chat")
        team1_message = FakeMessage()
        team2_message = FakeMessage()
        team2_message.channel = channel
...
class RerollRosterTests(HelperTestCase):
    ...
    async def test_persists_the_shuffle_and_edits_both_messages(self):
        ...
        team1_message = await self.channel.fetch_message(111)
        team2_message = await self.channel.fetch_message(112)
        self.assertEqual(team1_message.edit.await_count, 1)
        self.assertEqual(team2_message.edit.await_count, 1)
    ...
    async def test_turns_on_roster_use_roles_and_clears_stale_disliked_ids(self):
        self._seed_roster()
        self.helperObj.update(GUILD_ID, "roster_use_roles", 0)
        self.helperObj.update(GUILD_ID, "disliked_role_user_ids", "300,400")

        await self.helperObj._rerollRoster(GUILD_ID, self.channel)

        self.assertEqual(self.helperObj.get(GUILD_ID, "roster_use_roles"), 1)
        self.assertEqual(self.helperObj.get(GUILD_ID, "disliked_role_user_ids"), "")
...
class RosterActionViewTests(HelperTestCase):
    ...
    async def test_move_false_clears_a_previously_set_original_channel(self):
        ...
        # independent of which button eventually starts the game. A
        # leftover value from a prior Start game is possible too. Either
        # way, Start (no move) must override it so
    ...
    async def test_reroll_click_triggers_reroll(self):
        with patch.object(self.helperObj, "_rerollRoster", AsyncMock()) as reroll:
            click = self._click()
            await self.helperObj._handleRosterRerollClick(click)
        reroll.assert_awaited_once_with(GUILD_ID, self.channel)

    # roster_use_roles=0 used to reject a reroll click outright. Now Random
    # Roles is also how a never-role-labelled 5v5 roster turns roles on for
    # the first time, so it must still go through regardless.
    async def test_reroll_click_triggers_reroll_even_when_roster_never_used_roles(self):
        self.helperObj.update(GUILD_ID, "roster_use_roles", 0)
        with patch.object(self.helperObj, "_rerollRoster", AsyncMock()) as reroll:
            click = self._click()
            await self.helperObj._handleRosterRerollClick(click)
    ...
    # Regression: roster_starting is set the instant Start/Start (no
    # move) passes its checks, but roster_team2_message_id itself stays
    # intact (see _handleRosterStartClick's own comment) so
    # recordResult can still find the roster message later. Reroll/
    # Balanced Roles used to only check the message id, so they stayed
    # fully clickable on an already-started game. recordResult
    # reads team1/team2/disliked_role_user_ids live at result time, so a
    # late reroll would silently change who gets credited for the game
    # already in progress.
    async def test_reroll_click_rejected_once_the_game_has_started(self):
        self.helperObj.update(GUILD_ID, "roster_starting", 1)
        with patch.object(self.helperObj, "_rerollRoster", AsyncMock()) as reroll:
            click = self._click()
            await self.helperObj._handleRosterRerollClick(click)
    ...
    async def test_balance_roles_click_rejected_once_the_game_has_started(self):
        team1 = Team()
        team1.set_name("Team 1")
        for i in range(5):
            team1.add_player(Player(300 + i, f"A{i}"))
    ...
    async def test_balance_roles_click_rejected_on_a_stale_message(self):
        with patch.object(self.helperObj, "_applyBalancedRolesToRoster", AsyncMock()) as balance:
            click = self._click(message_id=999)
            await self.helperObj._handleRosterBalanceRolesClick(click)
        balance.assert_not_awaited()
    ...
    async def test_balance_roles_click_rejected_when_not_five_a_side(self):
        # setUp's default roster is 1v1, not the 5v5 shape roles apply to.
        with patch.object(self.helperObj, "_applyBalancedRolesToRoster", AsyncMock()) as balance:
            click = self._click()
            await self.helperObj._handleRosterBalanceRolesClick(click)
        balance.assert_not_awaited()
    ...
    async def test_balance_roles_click_triggers_assignment_when_five_a_side(self):
        team1 = Team()
        team1.set_name("Team 1")
        for i in range(5):
            team1.add_player(Player(300 + i, f"A{i}"))
        ...
        args = balance.await_args.args
        self.assertEqual(args[0], GUILD_ID)
        self.assertEqual(args[1], self.channel)
...
class RankedTeamHelperTests(HelperTestCase):
    ...
    async def test_use_roles_persists_who_got_a_disliked_role(self):
        ...
        # "forces disliked players" test). It picks members[0]/[1]
        # deterministically, since nobody has any other stated preference
        # to break the tie.
...
class RoleBalancedTeamAssignmentTests(HelperTestCase):
    ...
    def test_split_role_balanced_teams_minimizes_effective_elo_gap(self):
        ...
        # is the true minimum, so this test only passes if the brute force
        # actually searches all 32 combos instead of picking an arbitrary
        # one (or naively "same flip for every role").
        pairs = {
            "Top": [("A", 1100), ("B", 900)],
            "Jungle": [("C", 1100), ("D", 900)],
            "Mid": [("E", 1000), ("F", 1000)],
    ...
    def test_refine_role_balance_finds_a_better_partition_via_role_swap(self):
        ...
        # stuck at a forced 2000 gap. It can only pick which side each
        # *already-formed* pair's members land on, it can't swap who plays
        # what role. Swapping just one Jungle player for one Top player
        # (2000<->1000, 1000<->0) turns both pairs into a 1000/1000-ish mix
        # that *does* have a diff-0 split. That proves the swap step
        # actually looks past the initial partition instead of only
        # re-splitting it.
        members = self._members()
...
class FixedTeamRoleAssignmentTests(HelperTestCase):
    def _members(self, n, start_id):
        return [SimpleNamespace(id=start_id + i, name=f"M{start_id + i}") for i in range(n)]

    def test_assigns_each_teams_own_five_members_across_all_five_roles(self):
        team1_members = self._members(5, 900)
        team2_members = self._members(5, 910)

        team1_assigned, team2_assigned = self.helperObj._assignRolesForFixedTeams(
            GUILD_ID, [(m, 1000) for m in team1_members], [(m, 1000) for m in team2_members]
        )

        self.assertEqual([e[2] for e in team1_assigned], helper_module.SETUP_ROLE_NAMES)
        self.assertEqual([e[2] for e in team2_assigned], helper_module.SETUP_ROLE_NAMES)
        self.assertEqual({e[0].id for e in team1_assigned}, {m.id for m in team1_members})
        self.assertEqual({e[0].id for e in team2_assigned}, {m.id for m in team2_members})

    # Exercises the initial per-team fill directly (_assignRolesForBalance
    # with per_role=1), not the full _assignRolesForFixedTeams pipeline.
    # Once every player already has their liked role, refine's own
    # cross-team-diff-minimizing swaps are free to trade a "liked" pick
    # away in favor of closing the gap against the other (fixed, and in
    # this test unspecified) team. So asserting "still liked" only holds
    # for the fill step in isolation, not the whole pipeline's output.
    def test_prefers_each_players_liked_role_within_their_own_team(self):
        team1_members = self._members(5, 900)
        role_by_member = dict(zip((m.id for m in team1_members), helper_module.SETUP_ROLE_NAMES))
        for member in team1_members:
            self.helperObj._applySetupRolePreferences(GUILD_ID, member.id, [role_by_member[member.id]], [])

        assigned = self.helperObj._assignRolesForBalance(
            GUILD_ID, [(m, 1000) for m in team1_members], per_role=1
        )

        self.assertEqual(len(assigned), 5)
        for member, _elo, role, tier, _eff in assigned:
            self.assertEqual(role, role_by_member[member.id])
            self.assertEqual(tier, "liked")

    # Team1's naive fill forces its lone Jungle-disliker onto Jungle (worth
    # 800 effective instead of 900), while team2 is flat-neutral all the
    # way across (4500 total). That's an initial 100-point gap. Swapping
    # that player onto Top (which they have no opinion on) and moving
    # team1's Top player onto Jungle instead (also no opinion on it) makes
    # both team1 entries neutral, closing the gap to 0 without moving
    # anyone off team1 or touching team2 at all. That's exactly the
    # "membership can't move between sides, roles can" lever
    # _refineFixedTeamRoleBalance has.
    def test_refine_reduces_the_effective_elo_gap_between_the_two_fixed_teams(self):
        team1_members = self._members(5, 900)
        self.helperObj._applySetupRolePreferences(GUILD_ID, team1_members[0].id, [], ["Jungle"])
        team2_members = self._members(5, 910)

        # team1_members[0] (the Jungle-disliker) is deliberately placed on
        # Jungle itself. Everyone else gets whatever's left in
        # SETUP_ROLE_NAMES order.
        team1_roles = ["Jungle"] + [r for r in helper_module.SETUP_ROLE_NAMES if r != "Jungle"]
        team1_assigned = [
            self.helperObj._reassignRole(GUILD_ID, member, 1000, role)
            for member, role in zip(team1_members, team1_roles)
        ]
        team2_assigned = [
            self.helperObj._reassignRole(GUILD_ID, member, 1000, role)
            for member, role in zip(team2_members, helper_module.SETUP_ROLE_NAMES)
        ]
        self.assertEqual(abs(sum(e[4] for e in team1_assigned) - sum(e[4] for e in team2_assigned)), 100)

        refined1, refined2 = self.helperObj._refineFixedTeamRoleBalance(GUILD_ID, team1_assigned, team2_assigned)

        self.assertEqual(abs(sum(e[4] for e in refined1) - sum(e[4] for e in refined2)), 0)
        self.assertEqual({e[0].id for e in refined1}, {m.id for m in team1_members})
        role_counts = {}
        for entry in refined1:
            role_counts[entry[2]] = role_counts.get(entry[2], 0) + 1
        self.assertEqual(set(role_counts.values()), {1})


class ApplyBalancedRolesToRosterTests(HelperTestCase):
    def setUp(self):
        super().setUp()
        self.channel = FakeChannel("game-chat")
    ...
    async def test_assigns_roles_without_moving_anyone_between_teams(self):
        team1, team2 = self._seed_roster()

        await self.helperObj._applyBalancedRolesToRoster(GUILD_ID, self.channel, team1, team2)

        new_team1 = self.deserialize_team("team1")
        new_team2 = self.deserialize_team("team2")
        self.assertEqual({p.get_id() for p in new_team1.get_players()}, {300, 301, 302, 303, 304})
        self.assertEqual({p.get_id() for p in new_team2.get_players()}, {400, 401, 402, 403, 404})
        self.assertEqual(self.helperObj.get(GUILD_ID, "roster_use_roles"), 1)
        self.assertEqual(self.channel.fetch_message.await_count, 2)

    async def test_flags_disliked_role_placements(self):
        team1, team2 = self._seed_roster()
        # Every team1 player dislikes every role, so whichever role each
        # one lands on, it's a "disliked" placement.
        for player in team1.get_players():
            self.helperObj._applySetupRolePreferences(
                GUILD_ID, player.get_id(), [], helper_module.SETUP_ROLE_NAMES
            )
...
class CaptainsHelperTests(HelperTestCase):
    ...
    async def test_same_captain_rejected_before_any_state_change(self):
        ...
        ctx.response.send_message.assert_awaited_once_with("Mention two different people!", ephemeral=True)
    ...
    async def test_missing_captain_rejected_without_crashing(self):
        ...
        # ...) before checking for None, crashing with AttributeError
        # instead of showing this message.
        await self.helperObj.captainsHelper(ctx, captain1, None)

        ctx.response.send_message.assert_awaited_once_with("Mention two team captains!", ephemeral=True)
...
class NextDraftTurnTests(HelperTestCase):
    ...
    # Classic 2-side snake pattern: whoever picks pick N (1-indexed) follows
    ...
    # captain each to start, so team sizes 2/1 is "after pick 1"). The
    ...
    def test_returns_none_outside_snake_drafts(self):
        self.helperObj.update(GUILD_ID, "draft_snake", 0)
        team1, team2 = self._team(1), self._team(1)
        self.assertIsNone(self.helperObj._draftPickLabel(GUILD_ID, team1, team2))

    # Sizes here are the CURRENT (pre-pick) roster state. One captain each
    # to start (1, 1) is "about to make pick 1". The label describes the
    # pick that's about to happen, not one already made.
    def test_pick_label_across_first_eight_picks(self):
        self.helperObj.update(GUILD_ID, "draft_snake", 1)
        sizes_before_each_pick = [(1, 1), (2, 1), (2, 2), (2, 3), (3, 3), (4, 3), (4, 4), (4, 5)]
        expected_labels = [
            "Pick 1 of 1", "Pick 1 of 2", "Pick 2 of 2", "Pick 1 of 2",
            "Pick 2 of 2", "Pick 1 of 2", "Pick 2 of 2", "Pick 1 of 2",
        ]

        for (team1_size, team2_size), expected in zip(sizes_before_each_pick, expected_labels):
            team1, team2 = self._team(team1_size), self._team(team2_size)
            self.assertEqual(self.helperObj._draftPickLabel(GUILD_ID, team1, team2), expected)


class CaptainsDraftPickTests(HelperTestCase):
    def setUp(self):
        super().setUp()
        # Shared by every ctx below (the initial post and every later pick
        # click alike), the same way a real Discord button click's own
        # interaction.channel is always the channel its message actually
        # lives in. Picks need to land back in the SAME channel
        # captainsHelper first posted the roster embeds to, so
        # _updateDraftEmbeds' own fetch_message(id) can resolve anything.
        ...
        # to reference, the same way a None interaction.message would mean
        # for a real, no-longer-live Discord message.
    ...
    def _ctx(self, captain1, captain2, pool_members, user=None):
        ...
        user.voice = FakeVoiceState(voice_channel)
        # The draft-pick handlers resolve captains/pool members via
        # ctx.guild.members (not the voice channel's member list), so both
    ...
    async def test_captains_helper_posts_the_picker_view(self):
        ...
        view = last_call.kwargs["view"]
        self.assertIn(f"<@{captain1.id}>", content)
        self.assertIsInstance(view, helper_module.CaptainsDraftPickView)

    async def test_prompt_shows_no_pick_count_outside_snake_drafts(self):
        await self._draft_setup([(501, "Pool1"), (502, "Pool2")], snake=False)

        content, _ = self.helperObj._renderDraftPickView(GUILD_ID)

        self.assertNotIn("Pick", content)

    async def test_prompt_shows_pick_count_in_a_snake_draft(self):
        captain1, captain2, pool, ctx = await self._draft_setup(
            [(501, "Pool1"), (502, "Pool2"), (503, "Pool3")], snake=True
        )

        content, _ = self.helperObj._renderDraftPickView(GUILD_ID)
        self.assertIn("Pick 1 of 1", content)

        await self.helperObj._applyDraftPick(self._pick_ctx(captain1), pool[0], 1)
        content, _ = self.helperObj._renderDraftPickView(GUILD_ID)
        self.assertIn("Pick 1 of 2", content)

        await self.helperObj._applyDraftPick(self._pick_ctx(captain2), pool[1], 2)
        content, _ = self.helperObj._renderDraftPickView(GUILD_ID)
        self.assertIn("Pick 2 of 2", content)

    async def test_view_colors_player_buttons_by_turn(self):
        await self._draft_setup([(501, "Pool1"), (502, "Pool2")])

        _, view = self.helperObj._renderDraftPickView(GUILD_ID)
        self.assertTrue(all(b.item.style == discord.ButtonStyle.primary for b in self._slot_buttons(view)))

        self.helperObj.update(GUILD_ID, "turn", 2)
        _, view = self.helperObj._renderDraftPickView(GUILD_ID)
        self.assertTrue(all(b.item.style == discord.ButtonStyle.danger for b in self._slot_buttons(view)))

    async def test_random_button_is_always_present_and_green(self):
        await self._draft_setup([(501, "Pool1")])

        _, view = self.helperObj._renderDraftPickView(GUILD_ID)

        self.assertIn(view.random, view.children)
        self.assertEqual(view.random.style, discord.ButtonStyle.success)

    async def test_no_pagination_buttons_at_or_under_the_threshold(self):
        pool_entries = [(500 + i, f"Pool{i}") for i in range(helper_module.DRAFT_PICK_MAX_UNPAGINATED)]
        await self._draft_setup(pool_entries)

        _, view = self.helperObj._renderDraftPickView(GUILD_ID)

        self.assertEqual(len(self._slot_buttons(view)), helper_module.DRAFT_PICK_MAX_UNPAGINATED)
        self.assertNotIn(view.first, view.children)
        self.assertNotIn(view.prev, view.children)
        self.assertNotIn(view.next, view.children)
        self.assertNotIn(view.last, view.children)

    async def test_pagination_buttons_appear_over_the_threshold(self):
        pool_entries = [(500 + i, f"Pool{i}") for i in range(helper_module.DRAFT_PICK_MAX_UNPAGINATED + 1)]
        await self._draft_setup(pool_entries)

        _, view = self.helperObj._renderDraftPickView(GUILD_ID)

        self.assertEqual(len(self._slot_buttons(view)), helper_module.DRAFT_PICK_PAGE_SIZE)
        self.assertIn(view.first, view.children)
        self.assertIn(view.last, view.children)

    async def test_turn_enforced_wrong_captain(self):
        captain1, captain2, pool, ctx = await self._draft_setup([(501, "Pool1")])
        pick_ctx = self._pick_ctx(captain2)

        allowed = await self.helperObj._isDraftPickTurn(pick_ctx)

        self.assertFalse(allowed)
        pick_ctx.response.send_message.assert_awaited_once_with("Not your turn to pick.", ephemeral=True)

    async def test_non_captain_rejected(self):
        captain1, captain2, pool, ctx = await self._draft_setup([(501, "Pool1")])
        outsider = FakeMember("Outsider", id=999)
        pick_ctx = self._pick_ctx(outsider)
    ...
    async def test_stale_message_is_rejected(self):
        captain1, captain2, pool, ctx = await self._draft_setup([(501, "Pool1")])
        stale_message = FakeMessage(id=999999)
        pick_ctx = FakeInteraction(self.guild, captain1, channel=self.channel, message=stale_message)

        allowed = await self.helperObj._isDraftPickTurn(pick_ctx)

        self.assertFalse(allowed)
        pick_ctx.response.send_message.assert_awaited_once_with(
            "This draft is no longer active.", ephemeral=True
        )

    # Regression: a draft abandoned via /clear teams (or superseded by a
    # fresh draft) used to leave its old picker message fully clickable.
    # If the same person happened to be captain1 again in the new draft
    # (a common case: the same organizer running back-to-back games),
    # a click on the OLD message would resolve against the NEW draft's
    # pool instead of being rejected outright.
    async def test_a_superseded_drafts_old_picker_message_is_rejected(self):
        captain1, captain2, pool, ctx = await self._draft_setup([(501, "Pool1"), (502, "Pool2")])
        old_picker_message = self.picker_message

        # A brand new draft starts on the same guild, with the same
        # person as captain1 again (_draft_setup always uses id=401).
        await self._draft_setup([(601, "NewPool1"), (602, "NewPool2")])

        stale_click = FakeInteraction(
            self.guild, captain1, channel=self.channel, message=old_picker_message
        )
        allowed = await self.helperObj._isDraftPickTurn(stale_click)

        self.assertFalse(allowed)
        stale_click.response.send_message.assert_awaited_once_with(
            "This draft is no longer active.", ephemeral=True
        )

    async def test_valid_slot_click_adds_player_and_switches_turn(self):
        captain1, captain2, pool, ctx = await self._draft_setup([(501, "Pool1"), (502, "Pool2")])
        ...
        pick_ctx.response.edit_message.assert_awaited_once()
    ...
    async def test_slot_click_out_of_range_is_rejected(self):
        ...
        await self.helperObj._handleDraftPickSlotClick(pick_ctx, 5)

        pick_ctx.response.send_message.assert_awaited_once_with(
            "That player is no longer available.", ephemeral=True
        )
    ...
    async def test_pick_edits_the_existing_roster_embeds_instead_of_posting_new_ones(self):
        captain1, captain2, pool, ctx = await self._draft_setup([(501, "Pool1"), (502, "Pool2")])
        # captainsHelper's own initial post: team1 embed, team2 embed, PLAYERS embed.
        sends_before_pick = self.channel.send.await_count
        team1_message = await self.channel.fetch_message(self.helperObj.get(GUILD_ID, "roster_team1_message_id"))
        team2_message = await self.channel.fetch_message(self.helperObj.get(GUILD_ID, "roster_team2_message_id"))
        players_message = await self.channel.fetch_message(self.helperObj.get(GUILD_ID, "draft_players_message_id"))
        pick_ctx = self._pick_ctx(captain1)

        await self.helperObj._handleDraftPickSlotClick(pick_ctx, 0)

        # No new team/PLAYERS embeds posted for this pick, only the
        # existing three edited in place.
        self.assertEqual(self.channel.send.await_count, sends_before_pick)
        self.assertEqual(team1_message.edit.await_count, 1)
        self.assertEqual(team2_message.edit.await_count, 1)
    ...
    async def test_multiple_picks_keep_editing_the_same_messages(self):
        ...
        await self.helperObj._handleDraftPickSlotClick(self._pick_ctx(captain1), 0)
        await self.helperObj._handleDraftPickSlotClick(self._pick_ctx(captain2), 0)

        self.assertEqual(team1_message.edit.await_count, 2)
        self.assertEqual(team2_message.edit.await_count, 2)

    async def test_final_pick_reuses_the_same_messages_for_finalize_roster(self):
        captain1, captain2, pool, ctx = await self._draft_setup([(501, "Pool1")])
        team1_message = await self.channel.fetch_message(self.helperObj.get(GUILD_ID, "roster_team1_message_id"))
        team2_message = await self.channel.fetch_message(self.helperObj.get(GUILD_ID, "roster_team2_message_id"))
        sends_before_pick = self.channel.send.await_count
        ...
        # The only new send() is the "Both teams are set!" completion
        # announcement. _finalizeRoster attaches RosterActionView to
        # team2's message once drafting completes, and that has to be the
        # SAME message the roster was posted to originally, not a
        # freshly re-sent one.
        self.assertEqual(self.channel.send.await_count, sends_before_pick + 1)
        self.assertEqual(self.helperObj.get(GUILD_ID, "roster_team1_message_id"), team1_message.id)
        self.assertEqual(self.helperObj.get(GUILD_ID, "roster_team2_message_id"), team2_message.id)
        team2_message.edit.assert_any_call(view=ANY)

    async def test_players_embed_shows_everyone_drafted_once_pool_empties(self):
        captain1, captain2, pool, ctx = await self._draft_setup([(501, "Pool1")])
        players_message = await self.channel.fetch_message(self.helperObj.get(GUILD_ID, "draft_players_message_id"))

        await self.helperObj._handleDraftPickSlotClick(self._pick_ctx(captain1), 0)

        self.assertIn("Everyone has been drafted!", players_message.edit.call_args.kwargs["embed"].description)

    async def test_already_picked_player_rejected(self):
        captain1, captain2, pool, ctx = await self._draft_setup([(501, "Pool1"), (502, "Pool2")])
        pick_ctx1 = self._pick_ctx(captain1)
        await self.helperObj._handleDraftPickSlotClick(pick_ctx1, 0)  # Pool1 now on team1

        pick_ctx2 = self._pick_ctx(captain2)
        await self.helperObj._applyDraftPick(pick_ctx2, pool[0], 2)  # try to pick Pool1 again directly
    ...
    async def test_prompts_start_once_teams_reach_team_size_even_with_spectators_left(self):
        # Regression test: the old chooseHelper only ever checked whether
        # the whole draft pool was empty. A voice channel with more people
        # than team_size * 2 is expected to leave spectators undrafted, so
        # with 3 pool members and a team_size of 2 (1 pick needed per
        # team), both teams fill up after 2 picks while 1 spectator is
        ...
        pick_ctx2 = self._pick_ctx(captain2)
        await self.helperObj._handleDraftPickSlotClick(pick_ctx2, 0)  # pool re-indexed; team2 now size 2

        players = self.deserialize_team("players")
        self.assertEqual(len(players.get_players()), 1)  # one spectator still unpicked

        pick_ctx2.response.edit_message.assert_awaited_once()
        self.assertIsNone(pick_ctx2.response.edit_message.call_args.kwargs["view"])
    ...
    async def test_random_click_reports_when_pool_empty(self):
        ...
        self.helperObj.update(GUILD_ID, "captain2", Player(402, "Cap2").serializePlayer())
        ctx = self._pick_ctx(FakeMember("Cap1", id=401))

        await self.helperObj._handleDraftPickRandomClick(ctx)

        ctx.response.send_message.assert_awaited_once_with(
            "There are no players left to choose from!", ephemeral=True
        )

    async def test_random_click_picks_from_the_pool(self):
        captain1, captain2, pool, ctx = await self._draft_setup([(501, "Pool1")])
        pick_ctx = self._pick_ctx(captain1)

        await self.helperObj._handleDraftPickRandomClick(pick_ctx)

        team1 = self.deserialize_team("team1")
        self.assertIn(501, {p.get_id() for p in team1.get_players()})

    async def test_page_click_navigates_and_reflects_the_shrunk_page(self):
        pool_entries = [(500 + i, f"Pool{i}") for i in range(helper_module.DRAFT_PICK_PAGE_SIZE + 5)]
        captain1, captain2, pool, ctx = await self._draft_setup(pool_entries)
        pick_ctx = self._pick_ctx(captain1)

        await self.helperObj._handleDraftPickPageClick(pick_ctx, "next")

        self.assertEqual(self.helperObj.get(GUILD_ID, "draft_pick_page"), 1)
        pick_ctx.response.edit_message.assert_awaited_once()
        view = pick_ctx.response.edit_message.call_args.kwargs["view"]
        self.assertEqual(len(self._slot_buttons(view)), 5)  # remaining 5 players on page 2

    async def test_page_click_no_op_when_already_at_the_bound(self):
        pool_entries = [(500 + i, f"Pool{i}") for i in range(helper_module.DRAFT_PICK_MAX_UNPAGINATED + 1)]
        captain1, captain2, pool, ctx = await self._draft_setup(pool_entries)
        pick_ctx = self._pick_ctx(captain1)

        await self.helperObj._handleDraftPickPageClick(pick_ctx, "prev")  # already on page 0

        pick_ctx.response.defer.assert_awaited_once()
...
class ClearTeamsHelperTests(HelperTestCase):
    ...
    async def test_cancels_an_active_game_before_wiping_it(self):
        ...
        # still being bet on or played. Nothing was refunded, nobody was
        # moved back, and the eventual winner report would silently skip
...
class ResetEconomyHelperTests(HelperTestCase):
    ...
    async def test_a_given_user_id_only_wipes_that_players_row(self):
        self.helperObj.ensureEconomyRow(GUILD_ID, 901, "Alice")
        self.helperObj.ensureEconomyRow(GUILD_ID, 902, "Bob")
        self.cursor.execute(
            "UPDATE economy SET balance=500 WHERE guildId=?", (GUILD_ID,)
        )
        self.db.commit()

        self.helperObj.resetEconomyHelper(GUILD_ID, user_id=901)

        self.cursor.execute("SELECT COUNT(*) FROM economy WHERE guildId=? AND userId=?", (GUILD_ID, 901))
        self.assertEqual(self.cursor.fetchone()[0], 0)
        # Bob's row is untouched.
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 902, "balance"), 500)
...
class ResetEloHelperTests(HelperTestCase):
    ...
    async def test_a_given_user_id_only_resets_that_players_elo(self):
        self.helperObj.ensureEconomyRow(GUILD_ID, 901, "Alice")
        self.helperObj.ensureEconomyRow(GUILD_ID, 902, "Bob")
        self.helperObj.ensureGameStatsRow(GUILD_ID, 901, "Alice", "League")
        self.helperObj.ensureGameStatsRow(GUILD_ID, 902, "Bob", "League")
        self.cursor.execute(
            "UPDATE game_stats SET elo=1400 WHERE guildId=? AND game='League'", (GUILD_ID,)
        )
        self.db.commit()

        self.helperObj.resetEloHelper(GUILD_ID, user_id=901)

        self.assertEqual(self.helperObj.getGameStat(GUILD_ID, 901, "League", "elo"), helper_module.DEFAULT_ELO)
        # Bob's elo is untouched.
        self.assertEqual(self.helperObj.getGameStat(GUILD_ID, 902, "League", "elo"), 1400)
...
class CreateTournamentHelperTests(HelperTestCase):
    ...
    async def test_rejects_non_positive_team_size(self):
        ...
        ctx.response.send_message.assert_awaited_once_with("Team size must be greater than 0.", ephemeral=True)
        self.assertIsNone(self.helperObj.getTournament(GUILD_ID))

    async def test_rejects_too_few_teams(self):
        ctx = self._ctx()
        await self.helperObj.createTournamentHelper(ctx, "Cup", 5, 1, False)
        ctx.response.send_message.assert_awaited_once_with("A tournament needs at least 2 teams.", ephemeral=True)
        self.assertIsNone(self.helperObj.getTournament(GUILD_ID))
    ...
    async def test_overwriting_without_manage_guild_permission_is_rejected(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "Only a member with the Manage Server permission can overwrite an existing tournament.", ephemeral=True
        )
...
class CreateTeamHelperTests(HelperTestCase):
    ...
    async def test_rejects_duplicate_team_name(self):
        ...
        ctx2.response.send_message.assert_awaited_once_with(
            "A team named **Red** already exists in this server.", ephemeral=True
        )
...
class SaveTeamHelperTests(HelperTestCase):
    def _ctx(self, user_id=901, name="Alice"):
        return FakeInteraction(self.guild, FakeMember(name, id=user_id))

    def _seed_roster(self, column, player_ids_and_names):
        team = Team()
        team.set_name("Ephemeral")
        for pid, pname in player_ids_and_names:
            team.add_player(Player(pid, pname))
        self.helperObj.update(GUILD_ID, column, team.serializeTeam())

    async def test_rejects_when_there_is_no_roster_to_save(self):
        ctx = self._ctx()
        await self.helperObj.saveTeamHelper(ctx, 1, "Red")
        ctx.response.send_message.assert_awaited_once_with("There's no Team 1 from the last game to save.", ephemeral=True)
        self.assertIsNone(self.helperObj.getTeamRow(GUILD_ID, "Red"))

    async def test_rejects_a_caller_who_was_not_on_that_team(self):
        self._seed_roster("team1", [(902, "Bob"), (903, "Carol")])
        ctx = self._ctx()  # user_id=901, not in the seeded roster

        await self.helperObj.saveTeamHelper(ctx, 1, "Red")

        ctx.response.send_message.assert_awaited_once()
        args, kwargs = ctx.response.send_message.call_args
        self.assertIn("weren't part of Team 1", args[0])
        self.assertTrue(kwargs.get("ephemeral"))
        self.assertIsNone(self.helperObj.getTeamRow(GUILD_ID, "Red"))

    async def test_checks_only_the_requested_side(self):
        # Caller was rostered on team2, not team1. Asking to save "Team 1"
        # must still reject even though they played that game.
        self._seed_roster("team1", [(902, "Bob")])
        self._seed_roster("team2", [(901, "Alice")])
        ctx = self._ctx()

        await self.helperObj.saveTeamHelper(ctx, 1, "Red")

        self.assertIn("weren't part of Team 1", ctx.response.send_message.call_args.args[0])
        self.assertIsNone(self.helperObj.getTeamRow(GUILD_ID, "Red"))

    async def test_rejects_duplicate_team_name(self):
        await self.helperObj.createTeamHelper(self._ctx(user_id=999, name="Existing"), "Red", 5)
        self._seed_roster("team1", [(901, "Alice"), (902, "Bob")])
        ctx = self._ctx()
    ...
    async def test_saves_the_caller_as_captain_with_the_full_roster(self):
        self._seed_roster("team1", [(901, "Alice"), (902, "Bob"), (903, "Carol")])
        ctx = self._ctx()
        ...
        result = self.helperObj.getTeamRow(GUILD_ID, "Red")
        self.assertIsNotNone(result)
        _team_id, team = result
        self.assertTrue(self.helperObj.isTeamCaptain(team, 901))
        self.assertEqual({p.get_id() for p in team.get_players()}, {901, 902, 903})
        # Snapshotted as already full (not recruiting), see saveTeamHelper.
        self.assertEqual(team.get_team_size(), 3)
        self.assertEqual(team.get_size(), 3)

    async def test_does_not_modify_the_ephemeral_roster(self):
        self._seed_roster("team1", [(901, "Alice"), (902, "Bob")])
        ctx = self._ctx()

        await self.helperObj.saveTeamHelper(ctx, 1, "Red")

        still_there = self.deserialize_team("team1")
        self.assertEqual({p.get_id() for p in still_there.get_players()}, {901, 902})

    async def test_saving_team_2_reads_the_team2_column(self):
        self._seed_roster("team1", [(902, "Bob")])
        self._seed_roster("team2", [(901, "Alice"), (903, "Carol")])
        ctx = self._ctx()

        await self.helperObj.saveTeamHelper(ctx, 2, "Blue")

        _team_id, team = self.helperObj.getTeamRow(GUILD_ID, "Blue")
        self.assertEqual({p.get_id() for p in team.get_players()}, {901, 903})
...
class TeamRenameHelperTests(HelperTestCase):
    ...
    async def test_rejects_unknown_team(self):
        ...
        ctx.response.send_message.assert_awaited_once_with("No team named **Ghosts** in this server.", ephemeral=True)
    ...
    async def test_rejects_non_captain_non_admin(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "Only **Red**'s captain or a member with the Manage Server permission can rename it.", ephemeral=True
        )
        self.assertIsNotNone(self.helperObj.getTeamRow(GUILD_ID, "Red"))
    ...
    async def test_rejects_renaming_to_the_same_name(self):
        ...
        ctx.response.send_message.assert_awaited_once_with("**Red** is already named that.", ephemeral=True)
    ...
    async def test_rejects_a_name_already_taken_by_another_team(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "A team named **Blue** already exists in this server.", ephemeral=True
        )
        self.assertIsNotNone(self.helperObj.getTeamRow(GUILD_ID, "Red"))
...
class TeamTransferHelperTests(HelperTestCase):
    ...
    async def test_rejects_non_captain_non_admin(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "Only **Red**'s captain or a member with the Manage Server permission can transfer it.", ephemeral=True
        )
        _, team = self.helperObj.getTeamRow(GUILD_ID, "Red")
        self.assertTrue(self.helperObj.isTeamCaptain(team, 901))
    ...
    async def test_rejects_a_target_not_on_the_roster(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            f"{target.mention} isn't on **Red**'s roster; invite them with /team invite first.", ephemeral=True
        )
        _, team = self.helperObj.getTeamRow(GUILD_ID, "Red")
        self.assertTrue(self.helperObj.isTeamCaptain(team, 901))
    ...
    async def test_rejects_transferring_to_the_current_captain(self):
        ...
        ctx = self._ctx()
        await self.helperObj.teamTransferHelper(ctx, "Red", target)
        ctx.response.send_message.assert_awaited_once_with(f"{target.mention} is already **Red**'s captain.", ephemeral=True)

    async def test_captain_transferring_posts_an_offer_without_moving_captaincy_yet(self):
        await self._make_team("Red")
        self._add_to_roster("Red", 902, "Bob")
        target = FakeMember("Bob", id=902)
        ctx = self._ctx()
        ctx.original_response.return_value = FakeMessage(id=555)
        ...
        self.assertIsInstance(
            ctx.response.send_message.call_args.kwargs["view"], helper_module.TeamTransferAcceptView
        )

        # captaincy hasn't actually moved yet: it's still just an offer
        _, team = self.helperObj.getTeamRow(GUILD_ID, "Red")
        self.assertTrue(self.helperObj.isTeamCaptain(team, 901))
        self.assertFalse(self.helperObj.isTeamCaptain(team, 902))

        self.cursor.execute("SELECT toCaptainId FROM team_transfers WHERE messageId=555")
        self.assertEqual(self.cursor.fetchone(), (902,))

    async def test_non_captain_admin_can_also_start_a_transfer(self):
        await self._make_team("Red")
        self._add_to_roster("Red", 902, "Bob")
        ctx = self._ctx(user_id=903, name="Charlie", manage_guild=True)
        ctx.original_response.return_value = FakeMessage(id=556)

        await self.helperObj.teamTransferHelper(ctx, "Red", FakeMember("Bob", id=902))

        self.cursor.execute("SELECT COUNT(*) FROM team_transfers WHERE messageId=556")
        self.assertEqual(self.cursor.fetchone()[0], 1)

    async def test_rejects_a_second_transfer_while_one_is_already_pending(self):
        await self._make_team("Red")
        self._add_to_roster("Red", 902, "Bob")
        self._add_to_roster("Red", 903, "Cleo")

        first = self._ctx()
        first.original_response.return_value = FakeMessage(id=1)
        await self.helperObj.teamTransferHelper(first, "Red", FakeMember("Bob", id=902))

        second = self._ctx()
        await self.helperObj.teamTransferHelper(second, "Red", FakeMember("Cleo", id=903))

        second.response.send_message.assert_awaited_once_with(
            "**Red** already has a pending captaincy transfer. Cancel it first if you want to send "
            "a different one.",
            ephemeral=True,
        )
        self.cursor.execute("SELECT COUNT(*) FROM team_transfers")
        self.assertEqual(self.cursor.fetchone()[0], 1)

    async def test_force_transfers_immediately_for_an_admin(self):
        await self._make_team("Red")
        self._add_to_roster("Red", 902, "Bob")
        target = FakeMember("Bob", id=902)
        ctx = self._ctx()  # Alice: captain and manage_guild=True by default

        await self.helperObj.teamTransferHelper(ctx, "Red", target, force=True)

        ctx.response.send_message.assert_awaited_once_with(
            f"**Red**'s captaincy has been transferred to {target.mention}; no confirmation needed."
        )
        _, team = self.helperObj.getTeamRow(GUILD_ID, "Red")
        self.assertTrue(self.helperObj.isTeamCaptain(team, 902))
    ...
    async def test_force_is_rejected_for_a_captain_without_manage_guild(self):
        await self._make_team("Red")
        self._add_to_roster("Red", 902, "Bob")
        ctx = self._ctx(manage_guild=False)  # still the captain, just not an admin

        await self.helperObj.teamTransferHelper(ctx, "Red", FakeMember("Bob", id=902), force=True)
    ...
    async def test_transferred_captain_can_then_use_team_delete(self):
        # Regression: /team leave used to be a dead end for a captain
        # ("nobody to hand it to yet"). This confirms the old captain, no
        # longer captain after a transfer, isn't stuck on that team either.
        # force=True here since this test is about /team leave afterward,
        # not about the transfer offer/accept flow itself.
        await self._make_team("Red")
        self._add_to_roster("Red", 902, "Bob")
        await self.helperObj.teamTransferHelper(self._ctx(), "Red", FakeMember("Bob", id=902), force=True)
...
class TeamTransferAcceptViewTests(HelperTestCase):
    def setUp(self):
        super().setUp()
        self.channel = FakeChannel("general")
        self.helperObj.client = FakeClient(channels=[self.channel], guilds=[self.guild])

    async def _make_team_and_offer(self):
        ...
        transfer_ctx = FakeInteraction(self.guild, FakeMember("Alice", id=901), channel=self.channel)
        posted_message = FakeMessage(id=888)
        transfer_ctx.original_response.return_value = posted_message
        await self.helperObj.teamTransferHelper(transfer_ctx, "Red", FakeMember("Bob", id=902))
    ...
    async def test_accept_from_someone_other_than_the_offered_captain_is_rejected(self):
        message = await self._make_team_and_offer()
        view = helper_module.TeamTransferAcceptView(self.helperObj)
    ...
    async def test_accept_from_the_offered_captain_moves_captaincy(self):
        message = await self._make_team_and_offer()
        view = helper_module.TeamTransferAcceptView(self.helperObj)
        ...
        self.assertFalse(self.helperObj.isTeamCaptain(team, 901))
        click.response.send_message.assert_awaited_once()
        self.assertIn("captaincy", click.response.send_message.call_args.args[0])

        self.cursor.execute("SELECT COUNT(*) FROM team_transfers")
        self.assertEqual(self.cursor.fetchone()[0], 0)

    async def test_decline_from_the_offered_captain_leaves_captaincy_untouched(self):
        message = await self._make_team_and_offer()
        view = helper_module.TeamTransferAcceptView(self.helperObj)
        click = self._click(message, 902)
        await view.decline.callback(click)

        _, team = self.helperObj.getTeamRow(GUILD_ID, "Red")
        self.assertTrue(self.helperObj.isTeamCaptain(team, 901))
        click.response.send_message.assert_awaited_once()
        message_text = click.response.send_message.call_args.args[0]
        self.assertIn("declined", message_text)
        self.assertIn("Alice", message_text)

        self.cursor.execute("SELECT COUNT(*) FROM team_transfers")
        self.assertEqual(self.cursor.fetchone()[0], 0)

    async def test_decline_from_someone_other_than_the_offered_captain_is_rejected(self):
        message = await self._make_team_and_offer()
        view = helper_module.TeamTransferAcceptView(self.helperObj)
        click = self._click(message, 903)
        await view.decline.callback(click)
    ...
    async def test_cancel_from_the_captain_retracts_the_offer(self):
        message = await self._make_team_and_offer()
        view = helper_module.TeamTransferAcceptView(self.helperObj)
        click = self._click(message, 901, name="Alice")
        await view.cancelTransfer.callback(click)
        ...
        self.assertIsNone(click.response.edit_message.call_args.kwargs["view"])

    async def test_cancel_from_a_non_captain_non_admin_is_rejected(self):
        message = await self._make_team_and_offer()
        view = helper_module.TeamTransferAcceptView(self.helperObj)
        click = FakeInteraction(
            self.guild, FakeMember("Random", id=904, manage_guild=False), channel=self.channel, message=message
        )
        await view.cancelTransfer.callback(click)
    ...
    async def test_accept_after_already_used_up_is_handled_gracefully(self):
        message = await self._make_team_and_offer()
        view = helper_module.TeamTransferAcceptView(self.helperObj)
        await view.decline.callback(self._click(message, 902))

        click = self._click(message, 902)
        await view.accept.callback(click)

        self.assertTrue(click.response.send_message.call_args.kwargs.get("ephemeral"))
        self.assertIn("no longer pending", click.response.send_message.call_args.args[0])
...
class TeamDeleteHelperTests(HelperTestCase):
    ...
    async def test_rejects_non_captain_non_admin(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "Only **Red**'s captain or a member with the Manage Server permission can delete it.", ephemeral=True
        )
        self.assertIsNotNone(self.helperObj.getTeamRow(GUILD_ID, "Red"))
    ...
    async def test_timeout_marks_the_prompt_expired(self):
        await self._make_team("Red")
        ctx = self._ctx()
        posted_message = FakeMessage(id=556)
        ctx.original_response.return_value = posted_message
        await self.helperObj.teamDeleteHelper(ctx, "Red")
        view = ctx.response.send_message.call_args.kwargs["view"]
        ...
        self.assertIsNotNone(self.helperObj.getTeamRow(GUILD_ID, "Red"))
        posted_message.edit.assert_awaited_once()
        self.assertIn("expired", posted_message.edit.call_args.kwargs["content"])
        self.assertTrue(view.confirm.disabled)
...
class _FakeLogoDirTestCase(HelperTestCase):
    ...
    def setUp(self):
        ...
        # attaches one internally) leaves the fd open. Windows won't let a
        # directory delete out from under an open file the way POSIX does.
        ...
        # _drawMatchupColumn's random-logo fallback (see
...
class TeamSetHelperTests(_FakeLogoDirTestCase):
    ...
    async def test_rejects_unknown_team(self):
        ...
        ctx.response.send_message.assert_awaited_once_with("No team named **Nonexistent** in this server.", ephemeral=True)
    ...
    async def test_rejects_non_captain_non_admin(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "Only **Red**'s captain or a member with the Manage Server permission can change its settings.", ephemeral=True
        )
    ...
    async def test_rejects_an_unknown_logo_name(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "No logo named **NotALogo**; pick one from the autocomplete list.", ephemeral=True
        )
...
class TeamInviteHelperTests(HelperTestCase):
    ...
    async def test_rejects_non_captain_non_admin(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "Only **Red**'s captain or a member with the Manage Server permission can invite players.", ephemeral=True
        )
    ...
    async def test_rejects_inviting_a_bot(self):
        ...
        await self.helperObj.teamInviteHelper(ctx, "Red", [target])
        ctx.response.send_message.assert_awaited_once_with("You can't invite a bot to a team.", ephemeral=True)

    async def test_rejects_inviting_someone_already_on_the_team(self):
        await self._make_team()
        ctx = self._ctx()
        await self.helperObj.teamInviteHelper(ctx, "Red", [ctx.user])
        ctx.response.send_message.assert_awaited_once_with("Alice is already on **Red**.", ephemeral=True)

    async def test_rejects_reinviting_someone_with_a_pending_invite(self):
        await self._make_team()
        target = FakeMember("Bob", id=902)
        first = self._ctx()
        first.original_response.return_value = FakeMessage(id=777)
        await self.helperObj.teamInviteHelper(first, "Red", [target])

        second = self._ctx()
        await self.helperObj.teamInviteHelper(second, "Red", [target])

        second.response.send_message.assert_awaited_once_with(
            "Bob already has a pending invite to **Red**.", ephemeral=True
        )
        # Still just the one row from the first invite, not a second.
        self.cursor.execute("SELECT COUNT(*) FROM team_invites WHERE teamId=?", (
            self.helperObj.getTeamRow(GUILD_ID, "Red")[0],
        ))
        self.assertEqual(self.cursor.fetchone()[0], 1)

    async def test_force_bypasses_the_pending_invite_check(self):
        await self._make_team()
        target = FakeMember("Bob", id=902)
        first = self._ctx()
        first.original_response.return_value = FakeMessage(id=777)
        ...
        second = self._ctx()
        await self.helperObj.teamInviteHelper(second, "Red", [target], force=True)
    ...
    async def test_force_rejects_a_captain_who_is_not_also_an_admin(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            ...
            "everyone else still needs the invitee's own confirmation.", ephemeral=True
        )
        _, team = self.helperObj.getTeamRow(GUILD_ID, "Red")
        self.assertEqual([p.get_id() for p in team.get_players()], [901])
...
class TeamInviteAcceptViewTests(HelperTestCase):
    ...
    async def test_decline_from_someone_other_than_the_invitee_is_rejected(self):
        message = await self._make_team_and_invite()
        view = helper_module.TeamInviteAcceptView(self.helperObj)
        click = self._click(message, 903)
        await view.decline.callback(click)
    ...
    async def test_decline_from_invitee_removes_the_invite_without_joining(self):
        message = await self._make_team_and_invite()
        view = helper_module.TeamInviteAcceptView(self.helperObj)
        click = self._click(message, 902)
        await view.decline.callback(click)
        ...
        click.response.send_message.assert_awaited_once()
        self.assertIn("Bob", click.response.send_message.call_args.args[0])
        self.assertIn("declined", click.response.send_message.call_args.args[0])

    async def test_cancel_from_the_captain_retracts_the_whole_invite(self):
        message = await self._make_team_and_invite(
            [FakeMember("Bob", id=902), FakeMember("Cleo", id=903)]
        )
        view = helper_module.TeamInviteAcceptView(self.helperObj)
        click = self._click(message, 901, name="Alice")  # 901 is Red's captain
        await view.cancelInvite.callback(click)

        self.cursor.execute("SELECT COUNT(*) FROM team_invites WHERE messageId=?", (message.id,))
        self.assertEqual(self.cursor.fetchone()[0], 0)
        _, team = self.helperObj.getTeamRow(GUILD_ID, "Red")
        self.assertEqual({p.get_id() for p in team.get_players()}, {901})
        click.response.edit_message.assert_awaited_once()
        ...
        self.assertIn("Cleo", content)
        self.assertIsNone(click.response.edit_message.call_args.kwargs["view"])

    async def test_cancel_from_a_manage_guild_admin_who_isnt_the_captain_works(self):
        message = await self._make_team_and_invite()
        view = helper_module.TeamInviteAcceptView(self.helperObj)
        click = FakeInteraction(
            self.guild, FakeMember("Admin", id=999, manage_guild=True), channel=self.channel, message=message
        )
        await view.cancelInvite.callback(click)
    ...
    async def test_cancel_when_invite_already_used_up_is_handled_gracefully(self):
        message = await self._make_team_and_invite()
        view = helper_module.TeamInviteAcceptView(self.helperObj)
        await view.accept.callback(self._click(message, 902))
        ...
        self.assertTrue(click.response.send_message.call_args.kwargs.get("ephemeral"))
        self.assertIn("already been used up", click.response.send_message.call_args.args[0])

        self.cursor.execute("SELECT COUNT(*) FROM team_invites")
        self.assertEqual(self.cursor.fetchone()[0], 0)

    async def test_multiple_invitees_on_the_same_message_decline_independently(self):
        message = await self._make_team_and_invite(
            [FakeMember("Bob", id=902), FakeMember("Cleo", id=903)]
        )
        view = helper_module.TeamInviteAcceptView(self.helperObj)
        ...
        # Cleo's own invite (same messageId) is untouched by Bob's decline
        self.cursor.execute("SELECT targetId FROM team_invites WHERE messageId=?", (message.id,))
        self.assertEqual(self.cursor.fetchall(), [(903,)])
...
# A pending /team invite or /wager against challenge used to sit forever
# with no way to expire, only Accept/Decline (or, for a challenge before
# DuelAcceptView grew a Decline button, no way out at all). Both now expire
# after PENDING_INVITE_EXPIRY_SECONDS (see expireStalePendingInvites,
# called periodically by bot.py's expireInvitesTask).
class ExpireStalePendingInvitesTests(HelperTestCase):
    def _ctx(self, user_id=901, name="Alice"):
        return FakeInteraction(self.guild, FakeMember(name, id=user_id))

    def _backdate(self, table, seconds_ago):
        cutoff = int(time.time()) - seconds_ago
        self.cursor.execute(f"UPDATE {table} SET createdAt=?", (cutoff,))
        self.db.commit()

    async def test_expires_a_team_invite_past_the_deadline(self):
        await self.helperObj.createTeamHelper(self._ctx(), "Red", 5)
        await self.helperObj.teamInviteHelper(self._ctx(), "Red", [FakeMember("Bob", id=902)])
        self._backdate("team_invites", helper_module.PENDING_INVITE_EXPIRY_SECONDS + 60)

        invites_expired, duels_expired, transfers_expired = self.helperObj.expireStalePendingInvites()

        self.assertEqual(invites_expired, 1)
        self.assertEqual(duels_expired, 0)
        self.assertEqual(transfers_expired, 0)
        self.cursor.execute("SELECT COUNT(*) FROM team_invites")
        self.assertEqual(self.cursor.fetchone()[0], 0)

    async def test_keeps_a_team_invite_still_within_the_deadline(self):
        await self.helperObj.createTeamHelper(self._ctx(), "Red", 5)
        await self.helperObj.teamInviteHelper(self._ctx(), "Red", [FakeMember("Bob", id=902)])
        self._backdate("team_invites", helper_module.PENDING_INVITE_EXPIRY_SECONDS - 60)

        invites_expired, _, _ = self.helperObj.expireStalePendingInvites()

        self.assertEqual(invites_expired, 0)
        self.cursor.execute("SELECT COUNT(*) FROM team_invites")
        self.assertEqual(self.cursor.fetchone()[0], 1)

    async def test_a_null_created_at_counts_as_already_expired(self):
        await self.helperObj.createTeamHelper(self._ctx(), "Red", 5)
        await self.helperObj.teamInviteHelper(self._ctx(), "Red", [FakeMember("Bob", id=902)])
        self.cursor.execute("UPDATE team_invites SET createdAt=NULL")
        self.db.commit()
    ...
    async def test_expires_a_pending_duel_challenge_past_the_deadline(self):
        self.helperObj.ensureEconomyRow(GUILD_ID, 901, "Alice")
        self.cursor.execute("UPDATE economy SET balance=1000 WHERE guildId=? AND userId=?", (GUILD_ID, 901))
        self.db.commit()
        await self.helperObj.challengeDuelHelper(self._ctx(), FakeMember("Bob", id=902), 100)
        self._backdate("duels", helper_module.PENDING_INVITE_EXPIRY_SECONDS + 60)

        invites_expired, duels_expired, _ = self.helperObj.expireStalePendingInvites()

        self.assertEqual(invites_expired, 0)
        self.assertEqual(duels_expired, 1)
        self.cursor.execute("SELECT COUNT(*) FROM duels")
        self.assertEqual(self.cursor.fetchone()[0], 0)

    async def test_keeps_an_accepted_duel_no_matter_how_old(self):
        # Real gold is escrowed on an accepted (AWAITING_RESULT) duel -
        # never auto-expired, regardless of age.
        self.cursor.execute(
            "INSERT INTO duels(guildId, channelId, messageId, challengerId, challengerName, "
            "targetId, targetName, amount, state, createdAt) "
            "VALUES(?, ?, ?, 901, 'Alice', 902, 'Bob', 100, 'AWAITING_RESULT', ?)",
            (GUILD_ID, 111, 222, int(time.time()) - helper_module.PENDING_INVITE_EXPIRY_SECONDS - 60)
        )
        self.db.commit()

        _, duels_expired, _ = self.helperObj.expireStalePendingInvites()

        self.assertEqual(duels_expired, 0)
        self.cursor.execute("SELECT COUNT(*) FROM duels")
        self.assertEqual(self.cursor.fetchone()[0], 1)

    async def test_expires_a_pending_captaincy_transfer_past_the_deadline(self):
        await self.helperObj.createTeamHelper(self._ctx(), "Red", 5)
        team_id, team = self.helperObj.getTeamRow(GUILD_ID, "Red")
        team.add_player(Player(902, "Bob"))
        self.helperObj.updateTeamData(team_id, team)
        await self.helperObj.teamTransferHelper(self._ctx(), "Red", FakeMember("Bob", id=902))
        self._backdate("team_transfers", helper_module.PENDING_INVITE_EXPIRY_SECONDS + 60)

        invites_expired, duels_expired, transfers_expired = self.helperObj.expireStalePendingInvites()

        self.assertEqual(invites_expired, 0)
        self.assertEqual(duels_expired, 0)
        self.assertEqual(transfers_expired, 1)
        self.cursor.execute("SELECT COUNT(*) FROM team_transfers")
        self.assertEqual(self.cursor.fetchone()[0], 0)

    async def test_keeps_a_captaincy_transfer_still_within_the_deadline(self):
        await self.helperObj.createTeamHelper(self._ctx(), "Red", 5)
        team_id, team = self.helperObj.getTeamRow(GUILD_ID, "Red")
        team.add_player(Player(902, "Bob"))
        self.helperObj.updateTeamData(team_id, team)
        ...
        _, _, transfers_expired = self.helperObj.expireStalePendingInvites()
...
class TeamLeaveHelperTests(HelperTestCase):
    ...
    async def test_rejects_the_captain(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            ...
            "or /team delete if you want the team gone entirely.", ephemeral=True
        )
        _, team = self.helperObj.getTeamRow(GUILD_ID, "Red")
        self.assertEqual([p.get_id() for p in team.get_players()], [901])
    ...
    async def test_rejects_someone_not_on_the_team(self):
        ...
        await self.helperObj.teamLeaveHelper(ctx, "Red")
        ctx.response.send_message.assert_awaited_once_with("You're not on **Red**.", ephemeral=True)
...
# The captain/admin counterpart to /team leave. A rostered player who
# won't (or can't) run /team leave themselves used to have no way off a
# roster short of the whole team being deleted.
class TeamRemoveHelperTests(HelperTestCase):
    def _ctx(self, user_id=901, name="Alice", manage_guild=True):
        return FakeInteraction(self.guild, FakeMember(name, id=user_id, manage_guild=manage_guild))
    ...
    async def test_rejects_unknown_team(self):
        ctx = self._ctx()
        target = FakeMember("Bob", id=902)
        await self.helperObj.teamRemoveHelper(ctx, "Nonexistent", target)
        ctx.response.send_message.assert_awaited_once_with(
            "No team named **Nonexistent** in this server.", ephemeral=True
        )
    ...
    async def test_rejects_non_captain_non_admin(self):
        ...
        ctx = self._ctx(user_id=903, name="Cleo", manage_guild=False)
        target = FakeMember("Bob", id=902)
        await self.helperObj.teamRemoveHelper(ctx, "Red", target)

        ctx.response.send_message.assert_awaited_once_with(
            "Only **Red**'s captain or a member with the Manage Server permission can remove a "
            "player from the roster.", ephemeral=True
        )
        _, team = self.helperObj.getTeamRow(GUILD_ID, "Red")
        self.assertEqual(sorted(p.get_id() for p in team.get_players()), [901, 902])

    async def test_non_captain_admin_can_remove(self):
        await self._make_team()
        _, team = self.helperObj.getTeamRow(GUILD_ID, "Red")
        team.add_player(Player(902, "Bob"))
        self.helperObj.updateTeamData(team.get_id(), team)
        ...
        ctx.response.send_message.assert_awaited_once()
        self.assertIn("removed", ctx.response.send_message.call_args.args[0])
        _, team = self.helperObj.getTeamRow(GUILD_ID, "Red")
        self.assertEqual([p.get_id() for p in team.get_players()], [901])

    async def test_rejects_removing_the_captain(self):
        await self._make_team()
        ctx = self._ctx()  # Alice, the captain
        target = FakeMember("Alice", id=901)
        await self.helperObj.teamRemoveHelper(ctx, "Red", target)
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "Alice is **Red**'s captain; use /team transfer to hand off the captaincy first, "
            "or /team delete if you want the team gone entirely.", ephemeral=True
        )
        _, team = self.helperObj.getTeamRow(GUILD_ID, "Red")
        self.assertEqual([p.get_id() for p in team.get_players()], [901])
    ...
    async def test_rejects_someone_not_on_the_team(self):
        await self._make_team()
        ctx = self._ctx()  # the captain, allowed to remove
        target = FakeMember("Bob", id=902)
        await self.helperObj.teamRemoveHelper(ctx, "Red", target)
        ctx.response.send_message.assert_awaited_once_with("Bob isn't on **Red**.", ephemeral=True)

    async def test_captain_removes_a_non_captain_player_from_the_roster(self):
        await self._make_team()
        _, team = self.helperObj.getTeamRow(GUILD_ID, "Red")
        team.add_player(Player(902, "Bob"))
        self.helperObj.updateTeamData(team.get_id(), team)
        ...
        ctx.response.send_message.assert_awaited_once_with(f"{target.mention} has been removed from **Red**.")
        _, team = self.helperObj.getTeamRow(GUILD_ID, "Red")
        self.assertEqual([p.get_id() for p in team.get_players()], [901])

    async def test_removal_does_not_affect_other_rostered_players(self):
        await self._make_team()
        _, team = self.helperObj.getTeamRow(GUILD_ID, "Red")
        team_id = team.get_id()
        team.add_player(Player(902, "Bob"))
...
class SetupHelperTests(HelperTestCase):
    ...
    async def test_omitting_the_name_when_it_collides_asks_for_an_explicit_one(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            ...
            "/setup again with solo_team_name set to something else.", ephemeral=True
        )
        self.cursor.execute("SELECT COUNT(*) FROM setup_role_sessions")
        self.assertEqual(self.cursor.fetchone()[0], 0)
    ...
    async def test_rejects_a_solo_team_name_colliding_with_an_existing_team(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "A team named **Taken** already exists in this server; pick another name for your solo team.", ephemeral=True
        )
...
class RenderTeamCardImageTests(_FakeLogoDirTestCase):
    ...
    def _expectedReadableAccent(self, sampled_color):
        ...
        # That way these tests verify the real readability-boosted color
        # rather than a hand-computed guess that'd drift if the
        # darken/lighten constants ever change.
    ...
    def test_accent_color_is_sampled_from_the_teams_own_logo(self):
        ...
        # frame's own outline pixel. That's the same observable proof
        ...
        # It should actually differ from the raw sample here too. A forest
        # green this dark needs boosting against its own (darker still)
        # derived background, exactly the scenario the mechanism exists
        # for.
    ...
    def test_readable_accent_is_boosted_when_the_sampled_color_is_dark(self):
        ...
        # brightness comfortably above its floor of 20), but it would be
        # hard to read as header/label text against its own derived
        # background without _ensureReadableAccent stepping in.
        team = self._team()
        ...
        # vignette center by the configured minimum contrast. That's the
        # actual readability guarantee this whole mechanism exists to
        # provide.
        background_color = tuple(round(c * 0.28) for c in sampled_color)
        background_center = self.helperObj._lightenColor(background_color, 0.3)
...
class TeamListHelperTests(HelperTestCase):
    ...
    async def test_no_teams_at_all_sends_a_message(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "No teams have been created in this server yet!", ephemeral=True
        )
...
# cards:true mode: this is /team lookup's own one-team-full-stats-card-
# per-page rendering, but sourced from every team matching /team list's
# filters instead of one player's teams. Needs _FakeLogoDirTestCase since the
# underlying render (_renderTeamStatsEmbed) falls back to a random built-in
# logo for any team without one of its own, the same reason
# MyTeamsHelperTests needs it.
...
class TeamListPagingViewCardsModeTests(_FakeLogoDirTestCase):
    ...
    async def test_list_mode_and_cards_mode_views_dont_cross_wires(self):
        ...
        # guild must still render as a summary list, not a card. That
        # proves _handleTeamListPageClick actually branches per-message on
        # the stored `cards` flag rather than some shared/global mode.
        # This needs enough teams for list mode's own paging (10 per page)
        # to have a real second page to flip to. The 3 from asyncSetUp
        # alone aren't enough.
        for i in range(10):
...
class UseTeamsHelperTests(HelperTestCase):
    ...
    async def test_rejects_picking_the_same_team_twice(self):
        ...
        ctx.response.send_message.assert_awaited_once_with("Pick two different teams.", ephemeral=True)
    ...
    async def test_rejects_unknown_first_team(self):
        ...
        ctx.response.send_message.assert_awaited_once_with("No team named **Red** in this server.", ephemeral=True)

    async def test_rejects_unknown_second_team(self):
        await self.helperObj.createTeamHelper(self._ctx(901, "Alice"), "Red", 5)
        ctx = self._ctx()
        await self.helperObj.useTeamsHelper(ctx, "Red", "Blue", False)
        ctx.response.send_message.assert_awaited_once_with("No team named **Blue** in this server.", ephemeral=True)
...
class ReuseTeamsHelperTests(HelperTestCase):
    ...
    async def test_rejects_when_theres_nothing_to_reuse(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            ...
            "/make-teams saved.", ephemeral=True
        )
...
class RegisterTeamHelperTests(HelperTestCase):
    ...
    async def test_rejects_non_captain_non_admin(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            ...
            "for the tournament.", ephemeral=True
        )
    ...
    async def test_rejects_wrong_team_size(self):
        ...
        await self.helperObj.registerTeamHelper(ctx, "Red")
        ctx.response.send_message.assert_awaited_once_with(
            "**Red** has 2 player(s), but this tournament needs teams of exactly 3.", ephemeral=True
        )
    ...
    async def test_rejects_double_registration(self):
        ...
        ctx2.response.send_message.assert_awaited_once_with(
            "**Red** is already registered for this tournament.", ephemeral=True
        )
    ...
    async def test_rejects_when_bracket_is_full(self):
        ...
        ctx2.response.send_message.assert_awaited_once_with("This tournament's bracket is already full.", ephemeral=True)
...
# Registering the wrong team used to have no lightweight undo, only
# /tournament create again (wipes every team's registration, not just
# one) or /clear tournament (deletes the whole bracket/history).
class UnregisterTeamHelperTests(HelperTestCase):
    def _ctx(self, user_id=901, name="Alice", manage_guild=True):
        return FakeInteraction(self.guild, FakeMember(name, id=user_id, manage_guild=manage_guild))
    ...
    async def _make_team(self, team_name, captain_id, captain_name, size):
        ...
        await self.helperObj.createTeamHelper(ctx, team_name, size)
        team_id, team = self.helperObj.getTeamRow(GUILD_ID, team_name)
        # Filler ids are offset by captain_id so two different teams'
        # fillers never collide. register_team's own "no shared players"
        # check would otherwise reject the second team registered.
        while team.get_size() < size:
            team.add_player(Player(captain_id * 1000 + team.get_size(), f"Filler{team.get_size()}"))
        self.helperObj.updateTeamData(team_id, team)
        return team_id
    ...
    async def test_rejects_when_no_tournament_exists(self):
        await self._make_team("Red", 901, "Alice", 2)
        ctx = self._ctx()
        await self.helperObj.unregisterTeamHelper(ctx, "Red")
        ctx.response.send_message.assert_awaited_once_with(
            "No tournament set up for this server. Use /tournament create first.", ephemeral=True
        )
    ...
    async def test_rejects_unknown_team(self):
        self.helperObj.saveTournament(GUILD_ID, Tournament("Cup", 2, 4))
        ctx = self._ctx()
        await self.helperObj.unregisterTeamHelper(ctx, "Nonexistent")
        ctx.response.send_message.assert_awaited_once_with(
            "No team named **Nonexistent** in this server.", ephemeral=True
        )
    ...
    async def test_rejects_non_captain_non_admin(self):
        ...
        await self.helperObj.unregisterTeamHelper(ctx, "Red")
        ctx.response.send_message.assert_awaited_once_with(
            "Only **Red**'s captain or a member with the Manage Server permission can unregister it "
            "from the tournament.", ephemeral=True
        )

    async def test_rejects_a_team_that_was_never_registered(self):
        self.helperObj.saveTournament(GUILD_ID, Tournament("Cup", 2, 4))
        await self._make_team("Red", 901, "Alice", 2)
        ctx = self._ctx()
        await self.helperObj.unregisterTeamHelper(ctx, "Red")
        ctx.response.send_message.assert_awaited_once_with(
            "**Red** isn't registered for this tournament.", ephemeral=True
        )

    async def test_rejects_once_a_bracket_has_been_built(self):
        self.helperObj.saveTournament(GUILD_ID, Tournament("Cup", 2, 4))
        await self._make_team("Red", 901, "Alice", 2)
        await self._make_team("Blue", 902, "Bob", 2)
        await self.helperObj.registerTeamHelper(self._ctx(), "Red")
        await self.helperObj.registerTeamHelper(self._ctx(user_id=902, name="Bob"), "Blue")
        tournament = self.helperObj.getTournament(GUILD_ID)
        tournament.set_bracket(self.helperObj.buildBracket(tournament.get_teams()))
        self.helperObj.saveTournament(GUILD_ID, tournament)
        ...
        ctx.response.send_message.assert_awaited_once()
        self.assertIn("bracket has already been built", ctx.response.send_message.call_args.args[0])
        # still registered: nothing changed
        self.assertEqual(len(self.helperObj.getTournament(GUILD_ID).get_teams()), 2)

    async def test_non_captain_admin_can_unregister(self):
        self.helperObj.saveTournament(GUILD_ID, Tournament("Cup", 2, 4))
        await self._make_team("Red", 901, "Alice", 2)
        await self.helperObj.registerTeamHelper(self._ctx(), "Red")
        ...
        ctx.response.send_message.assert_awaited_once()
        self.assertIn("unregistered", ctx.response.send_message.call_args.args[0])

    async def test_successful_unregistration_saves_and_confirms(self):
        self.helperObj.saveTournament(GUILD_ID, Tournament("Cup", 2, 4))
        await self._make_team("Red", 901, "Alice", 2)
        await self._make_team("Blue", 902, "Bob", 2)
        await self.helperObj.registerTeamHelper(self._ctx(), "Red")
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "**Red** unregistered from **Cup**. (1/4 teams)"
        )
        tournament = self.helperObj.getTournament(GUILD_ID)
        self.assertEqual([t.get_name() for t in tournament.get_teams()], ["Blue"])

    async def test_unregistering_frees_up_a_full_bracket_slot(self):
        self.helperObj.saveTournament(GUILD_ID, Tournament("Cup", 2, 1))
        await self._make_team("Red", 901, "Alice", 2)
        await self._make_team("Blue", 902, "Bob", 2)
        await self.helperObj.registerTeamHelper(self._ctx(), "Red")
        ...
        blue_ctx = self._ctx(user_id=902, name="Bob")
        await self.helperObj.registerTeamHelper(blue_ctx, "Blue")
        blue_ctx.response.send_message.assert_awaited_once_with(
            "This tournament's bracket is already full.", ephemeral=True
        )

        await self.helperObj.unregisterTeamHelper(self._ctx(), "Red")

        blue_ctx2 = self._ctx(user_id=902, name="Bob")
        await self.helperObj.registerTeamHelper(blue_ctx2, "Blue")
        blue_ctx2.response.send_message.assert_awaited_once_with("**Blue** registered for **Cup**! (1/1 teams)")

    async def test_unregistering_a_team_lets_a_shared_player_register_elsewhere(self):
        # register_team's "no shared players across registered teams"
        # check reads live off tournament.get_teams(), so freeing a slot
        # via unregister should un-block a player who was previously
        # rejected for being on another registered team.
        self.helperObj.saveTournament(GUILD_ID, Tournament("Cup", 2, 4))
        await self._make_team("Red", 901, "Alice", 2)
        await self._make_team("Blue", 902, "Bob", 2)
        ...
        blue_ctx = self._ctx(user_id=902, name="Bob")
        await self.helperObj.registerTeamHelper(blue_ctx, "Blue")
        blue_ctx.response.send_message.assert_awaited_once_with("**Blue** registered for **Cup**! (1/4 teams)")
...
class BuildBracketTests(HelperTestCase):
    ...
    def test_byes_are_never_paired_against_each_other(self):
        ...
        # report. This runs it many times (buildBracket shuffles) to catch
        # that regardless of where randomization happens to place things.
...
class ClearTournamentMatchesForGuildTests(HelperTestCase):
    ...
    async def test_does_not_restart_the_sequence_while_another_guild_still_has_rows(self):
        ...
        new_id = self._insert_match(GUILD_ID)
        # This continues past the other guild's highest id instead of
        # colliding with it. _settleMatchWagers and the concurrent-
        # betting-close timer both key off matchId alone, with no guildId
        # in their WHERE clause, so a reused id could settle or close out
        # that guild's still-live match.
...
class BracketImageTests(HelperTestCase):
    ...
    def test_grand_finals_image_only_appears_once_game_one_is_played(self):
        ...
        # It's sent as its own separate message (see _sendBracketText),
        # and only once Grand Finals has actually been played, so it's
        # covered here via _renderGrandFinalsImage directly instead.
    ...
    def test_losers_bracket_with_fresh_drop_in_leaves_renders_without_crashing(self):
        ...
        # bracket byes into the losers bracket, which used to need
        # special-case leading-blank padding in the old ASCII renderer.
        # The pixel-based layout needs no such handling, but this still
        # guards against it silently breaking again. It also spans both
        # sides of the two-sided-layout threshold (8 and 16+ teams).
    ...
    def test_losers_bracket_uses_two_sided_layout_once_large_enough(self):
        ...
        # This isn't a precise assertion, just a sanity check that
        # something structurally different is actually happening once the
        # layout switches over. (The single-sided renderer would instead
        # keep growing taller as team count increases, same as the
        # winners-bracket equivalent test above.)
...
class GrandFinalsImageDimmingTests(HelperTestCase):
    ...
    # Intercepts every draw.text(...) call the real render makes (it
    # wraps the actual method so the image still renders normally) and
    # returns {label_text: fill_color}. That way the fix can be checked
    # against what actually got drawn instead of re-deriving the same
...
class PrintBracketHelperTests(HelperTestCase):
    ...
    async def test_prints_the_bracket(self):
        ...
        text = ctx.edit_original_response.call_args.kwargs.get("content")
        self.assertIn("Cup", text)

        files = ctx.edit_original_response.call_args.kwargs.get("attachments")
        self.assertEqual(len(files), 1)
        self.assertEqual(files[0].filename, "winners_bracket.png")

    # Regression test: rendering a full bracket can outrun Discord's ~3s
    # ack window. The interaction has to be acknowledged with a
    # placeholder immediately, before the (potentially slow) render even
    # starts, not after.
    async def test_acknowledges_immediately_with_a_placeholder(self):
        tournament = Tournament("Cup", 2, 4)
        red, blue = Team(), Team()
        red.set_name("Red")
        blue.set_name("Blue")
        ...
        ctx.response.send_message.assert_awaited_once_with("Creating bracket, please wait...")
...
class RenderMatchupImageTests(_FakeLogoDirTestCase):
    def _team(self, name, captain_id=None, captain_name=None, extra_players=()):
        team = Team()
        team.set_name(name)
        # extra_players are added BEFORE the captain, so a naive "first
        # player in the list" reading would get this wrong. _orderedRoster
        # has to actually float the captain to the front, not just happen
        # to already find them there.
        for player_id, player_name in extra_players:
            team.add_player(Player(player_id, player_name))
    ...
    def test_team_with_no_logo_gets_a_random_built_in_one_instead_of_a_bare_ring(self):
        # /make-teams random, /make-teams draft, etc. build ad-hoc Team
        # objects that never go through _ensureLogo (that's only ever
        # called for persistent teams), so team.get_logo_path() is None
        # for them. This is what
...
class MatchupRoleIconTests(_FakeRoleIconDirTestCase):
    ...
    def test_missing_icon_files_degrade_to_no_extra_width(self):
        ...
        # expected files (a dev checkout that hasn't added the assets
        # yet). use_roles=True must not crash, and must not reserve width
        # for icons it can't actually draw.
        team1 = self._team_of_five("Red")
        team2 = self._team_of_five("Blue", captain_id=902, captain_name="Bob")
...
class StartTournamentHelperTests(HelperTestCase):
    ...
    async def test_rejects_when_already_finished(self):
        ...
        await self.helperObj.startTournamentHelper(ctx, "sequential")
        ctx.response.send_message.assert_awaited_once_with(
            "**Cup** is already finished; **Red** is the champion!", ephemeral=True
        )
...
# still delegating to the real asyncio.to_thread. That way the rest of each
# test's own assertions exercise genuinely-threaded, not mocked-away,
# behavior.
# ===========================================================================
...
class ImageRenderThreadOffloadTests(HelperTestCase):
    ...
    async def test_casual_matchup_image_is_offloaded(self):
        ...
        # real file, not just a recorded call. That proves wraps= genuinely
    ...
    async def test_grand_finals_drawing_is_offloaded_but_its_db_read_is_not(self):
        ...
        # never handed to asyncio.to_thread at all, since self.cursor is
    ...
    async def test_preview_image_render_is_offloaded(self):
        ...
        # a directory delete out from under an open file the way POSIX
        # does (the same reason _FakeLogoDirTestCase's own temp dir uses
        # this).
    ...
    async def test_render_does_not_block_the_event_loop(self):
        # Proves the offload is genuine, not just present in the call
        # graph. A slow render (blocked here on a threading.Event, since
        # it runs in a worker thread, not on the event loop) must not stop
        # an unrelated coroutine from making progress on the loop while
        # it's in flight.
    ...
    async def test_simultaneous_round_offloads_a_separate_render_per_match(self):
        ...
        # its own independent asyncio.to_thread call, and each ends up
        # with the correct, un-mixed-up pair of teams rather than one
        # match's render bleeding into another's.
        red = _captained_team("Red", 901, "Alice")
        blue = _captained_team("Blue", 902, "Bob")
        cleo = _captained_team("Cleo Team", 903, "Cleo")
        dan = _captained_team("Dan Team", 904, "Dan")
        ...
        # Each match's own pair of teams reached its own render call
        # intact. team1/team2 are args[2]/args[3] (args[1] is the
        # match_id). Who actually plays whom is random (buildBracket
        # shuffles seeding), so this checks the pairing is internally
        # consistent rather than asserting one specific bracket. Two
        # disjoint pairs, together covering all four teams exactly once,
        # is what "no cross-match mix-up" actually means here.
...
class TournamentReadyAndReportViewTests(HelperTestCase):
    ...
    async def test_report_click_rejects_a_bystander(self):
        red = _captained_team("Red", 901, "Alice")
        blue = _captained_team("Blue", 902, "Bob")
        await self._start("simultaneous", red, blue)
        match_id, message_id = self._only_match()
    ...
    async def test_report_click_allows_a_rostered_non_admin_captain(self):
        red = _captained_team("Red", 901, "Alice")
        blue = _captained_team("Blue", 902, "Bob")
        await self._start("simultaneous", red, blue)
        match_id, message_id = self._only_match()

        click = self._click(message_id, 901)
        click.user.guild_permissions.manage_guild = False
        view = helper_module.TournamentMatchReportView(self.helperObj)
        await view.team1.callback(click)
    ...
    # discord.py calls interaction_check before ever routing to Confirm's
    # own callback. This is what actually exercises the confirm-time gate
    # (separate from the initial report click's own check above).
    async def test_confirm_view_interaction_check_rejects_a_bystander(self):
        red = _captained_team("Red", 901, "Alice")
        blue = _captained_team("Blue", 902, "Bob")
        await self._start("simultaneous", red, blue)
        match_id, _message_id = self._only_match()

        view = helper_module.ConfirmTournamentMatchReportView(self.helperObj, GUILD_ID, match_id, 1, self.channel.id)
        click = FakeInteraction(self.guild, FakeMember("Bystander", id=999), channel=self.channel)
        click.user.guild_permissions.manage_guild = False

        self.assertFalse(await view.interaction_check(click))
        self.assertTrue(click.response.send_message.call_args.kwargs.get("ephemeral"))

    async def test_confirm_view_interaction_check_allows_a_rostered_captain(self):
        red = _captained_team("Red", 901, "Alice")
        blue = _captained_team("Blue", 902, "Bob")
        await self._start("simultaneous", red, blue)
        match_id, _message_id = self._only_match()
        ...
        view = helper_module.ConfirmTournamentMatchReportView(self.helperObj, GUILD_ID, match_id, 1, self.channel.id)
        click = FakeInteraction(self.guild, FakeMember("Alice", id=901), channel=self.channel)
        click.user.guild_permissions.manage_guild = False
    ...
    async def test_confirming_deletes_the_match_report_and_confirmation_messages(self):
        ...
        # message, the same object _confirm_report's view was built with
        # as report_message.
        click.message.delete.assert_awaited_once()
...
class CorrectTournamentMatchHelperTests(HelperTestCase):
    ...
    async def test_rejects_unknown_match(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "No tournament match with id 9999 in this server.", ephemeral=True
        )
    ...
    async def test_rejects_unresolved_match(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(f"Match #{match_id} hasn't been resolved yet.", ephemeral=True)
    ...
    async def test_rejects_already_correct_team(self):
        ...
        await self.helperObj.reportCorrectWinnerHelper(ctx, 1, match_id=match_id)
        ctx.response.send_message.assert_awaited_once_with(
            f"Match #{match_id} is already recorded as Team 1.", ephemeral=True
        )
    ...
    async def test_successful_correction_flips_bracket_and_winner(self):
        ...
        # not applied yet: only a confirmation prompt posted
        ctx.response.send_message.assert_awaited_once()
        view = ctx.response.send_message.call_args.kwargs["view"]
        ...
        self.assertIn("corrected", click.response.edit_message.call_args.kwargs["content"])
    ...
    async def test_correcting_a_match_reverses_and_reapplies_wager_payouts(self):
        ...
        await self.helperObj.reportCorrectWinnerHelper(ctx, 2, match_id=match_id)
        view = ctx.response.send_message.call_args.kwargs["view"]
        confirm_click = self._ctx()
        await view.confirm.callback(confirm_click)

        self.assertIn("reversed and reapplied", confirm_click.response.edit_message.call_args.kwargs["content"])
    ...
    async def test_correcting_a_match_with_no_wagers_has_no_wager_note(self):
        ...
        click = self._ctx()
        await view.confirm.callback(click)
        message = click.response.edit_message.call_args.kwargs["content"]
        self.assertNotIn("reversed", message)

    # Regression: /set correct-winner's match_id path used to reverse and
    # reapply payouts and re-propagate the bracket the instant the command
    # ran, no confirmation at all. Same one-accidental-click risk
    # every other winner-report flow in the bot already guards against.
    async def test_cancelling_leaves_the_match_and_bracket_untouched(self):
        match_id = await self._resolved_match(winner=1)
        ctx = self._ctx()
        await self.helperObj.reportCorrectWinnerHelper(ctx, 2, match_id=match_id)
        view = ctx.response.send_message.call_args.kwargs["view"]
    ...
    async def test_confirming_after_the_next_round_started_refuses_instead_of_applying(self):
        # The "next round already started" guard is re-checked at Confirm
        # time too, not just when the prompt was first built, in case a
        # round started in the gap between the two.
        match_id = await self._resolved_match(winner=1)
        ctx = self._ctx()
        await self.helperObj.reportCorrectWinnerHelper(ctx, 2, match_id=match_id)
        view = ctx.response.send_message.call_args.kwargs["view"]
        ...
        self.cursor.execute("SELECT winner FROM tournament_matches WHERE id=?", (match_id,))
        self.assertEqual(self.cursor.fetchone()[0], 1)
        self.assertIn("changed", click.response.edit_message.call_args.kwargs["content"])
    ...
    async def test_rejects_correcting_a_losers_bracket_match(self):
        ...
        await self.helperObj.reportCorrectWinnerHelper(ctx, 2, match_id=match_id)
        ctx.response.send_message.assert_awaited_once_with(
            f"Match #{match_id} is a losers bracket match. Correcting those isn't supported yet.", ephemeral=True
        )
    ...
    async def test_rejects_correcting_a_grand_finals_match(self):
        ...
        await self.helperObj.reportCorrectWinnerHelper(ctx, 2, match_id=match_id)
        ctx.response.send_message.assert_awaited_once_with(
            f"Match #{match_id} is a Grand Finals match. Correcting those isn't supported yet.", ephemeral=True
        )
...
class BuildLosersBracketTests(HelperTestCase):
    ...
    def test_playing_it_out_always_reaches_a_losers_champion(self):
        ...
        # non-power-of-two counts that feed winners-bracket byes into the
...
class CreateBracketHelperDoubleEliminationTests(HelperTestCase):
    ...
    async def test_stale_match_rows_are_cleared_once_the_rebuild_is_confirmed(self):
        ...
        # Existing match rows mean this rebuild is destructive, so it goes
        # through ConfirmBracketOverwriteView now instead of clearing
        # immediately. See CreateBracketOverwriteConfirmationTests.
        tournament = Tournament("Cup", 1, 2)
        tournament.register_team(self._team("Red"))
        tournament.register_team(self._team("Blue"))
        self.helperObj.saveTournament(GUILD_ID, tournament)
...
# /tournament create-bracket used to erase any existing match history
# outright, no confirmation and reachable by anyone, the moment it was run
# a second time. Mirrors CreateTournamentHelperTests' own overwrite-
# confirmation coverage for /tournament create.
class CreateBracketOverwriteConfirmationTests(HelperTestCase):
    def _ctx(self, user_id=901, name="Alice", manage_guild=True):
        return FakeInteraction(self.guild, FakeMember(name, id=user_id, manage_guild=manage_guild))
    ...
    def _tournament_with_match_history(self):
        tournament = Tournament("Cup", 1, 2)
        tournament.register_team(self._team("Red"))
        tournament.register_team(self._team("Blue"))
        self.helperObj.saveTournament(GUILD_ID, tournament)
    ...
    async def test_no_match_history_needs_neither_permission_nor_confirmation(self):
        tournament = Tournament("Cup", 1, 2)
        tournament.register_team(self._team("Red"))
        tournament.register_team(self._team("Blue"))
        self.helperObj.saveTournament(GUILD_ID, tournament)
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "Bracket created for **Cup** - 2 teams, single elimination."
        )

    async def test_rebuilding_without_manage_guild_permission_is_rejected(self):
        self._tournament_with_match_history()
        ctx = self._ctx(manage_guild=False)
    ...
    async def test_match_history_requires_confirmation_before_rebuilding(self):
        self._tournament_with_match_history()
        ctx = self._ctx()
        posted = FakeMessage(id=555)
        ctx.original_response.return_value = posted
        ...
        ctx.response.send_message.assert_awaited_once()
        text = ctx.response.send_message.call_args.args[0]
        self.assertIn("match history", text)
        view = ctx.response.send_message.call_args.kwargs["view"]
        self.assertIs(view.message, posted)
        ...
        self.cursor.execute("SELECT COUNT(*) FROM tournament_matches WHERE guildId=?", (GUILD_ID,))
        self.assertEqual(self.cursor.fetchone()[0], 1)
        self.assertFalse(self.helperObj.getTournament(GUILD_ID).is_double_elimination())

    async def test_confirming_rebuild_replaces_the_bracket_and_clears_match_history(self):
        self._tournament_with_match_history()
        ctx = self._ctx()
        await self.helperObj.createBracketHelper(ctx, True)
        view = ctx.response.send_message.call_args.kwargs["view"]
        ...
        restored = self.helperObj.getTournament(GUILD_ID)
        self.assertTrue(restored.is_double_elimination())
        self.assertGreater(len(restored.get_bracket()), 0)
        click.response.edit_message.assert_awaited_once()
        self.assertIn("Bracket rebuilt", click.response.edit_message.call_args.kwargs["content"])

    async def test_cancelling_rebuild_keeps_the_existing_bracket_and_match_history(self):
        self._tournament_with_match_history()
        ctx = self._ctx()
        await self.helperObj.createBracketHelper(ctx, True)
        view = ctx.response.send_message.call_args.kwargs["view"]
    ...
    async def test_rebuild_confirmation_rejects_non_invoker(self):
        self._tournament_with_match_history()
        ctx = self._ctx()
        await self.helperObj.createBracketHelper(ctx, True)
        view = ctx.response.send_message.call_args.kwargs["view"]
...
class DoubleEliminationMatchFlowTests(HelperTestCase):
    ...
    async def test_tournament_completion_sends_the_grand_finals_bracket_image(self):
        ...
        # some earlier round's bracket reprint that happened to also
        # carry a (pre-finals) losers-bracket image.
    ...
    async def test_match_results_update_each_teams_persisted_win_loss_record(self):
        ...
        # the PERSISTED team row (see _recordMatchResult), or the
        # leaderboard would never show anything but 0W-0L. This exercises
        # all three (winners bracket, losers bracket, and Grand Finals all
        # get played out here) in one pass.
        red = _captained_team("Red", 901, "Alice")
        blue = _captained_team("Blue", 902, "Bob")
        cleo = _captained_team("Cleo Team", 903, "Cleo")
        dan = _captained_team("Dan Team", 904, "Dan")
    ...
    async def test_bracket_reset_when_losers_champion_wins_game_one(self):
        ...
        def winner_of(team1, team2, bracket_type, round_index):
            ...
            # Everywhere else (including the reset match), Red always
            # wins. Red stays undefeated through the winners bracket, then
            # takes the reset to become champion after dropping game 1.
...
class InterleavedLosersBracketTimingTests(HelperTestCase):
    ...
    # Finals match row (guarding against _startGrandFinals firing more
    # than once now that multiple code paths can reach it).
...
class MakeEmbedStringTests(HelperTestCase):
    # A username with markdown characters (_, *) used to get parsed as
    # italics/bold once dropped into an embed description next to every
    # other player's name. makeEmbedString now escapes them so the raw
    # username text renders as-is.
    async def test_escapes_markdown_characters_in_plain_names(self):
        team = Team()
        team.add_player(Player(1, "under_score"))
        team.add_player(Player(2, "star*person"))

        result = self.helperObj.makeEmbedString(team)

        self.assertIn("under\\_score", result)
        self.assertIn("star\\*person", result)

    async def test_escapes_markdown_characters_in_role_labelled_names(self):
        team = Team()
        for i in range(5):
            team.add_player(Player(700 + i, f"p_{i}"))

        result = self.helperObj.makeEmbedString(team, useRoles=True)

        for i in range(5):
            self.assertIn(f"p\\_{i}", result)
...
class PrintEmbedTests(HelperTestCase):
    ...
    async def test_returns_the_two_distinct_posted_messages(self):
        ...
        team1_message, team2_message, players_message = await self.helperObj.printEmbed(ctx, team1, team2)

        # A fresh message per send() call, not one shared object. This
        # matters since _finalizeRoster only ever reacts to the second one.
        self.assertIsNot(team1_message, team2_message)
        self.assertEqual(ctx.channel.send.await_count, 2)
        self.assertIsNone(players_message)

    async def test_returns_the_posted_players_message_when_a_pool_is_given(self):
        team1 = self._five_player_team("Team 1", 700)
        team2 = self._five_player_team("Team 2", 800)
        pool = Team()
        pool.add_player(Player(900, "Spectator"))
        ctx = FakeInteraction(self.guild, FakeMember("Caller"))

        team1_message, team2_message, players_message = await self.helperObj.printEmbed(
            ctx, team1, team2, pool
        )

        self.assertEqual(ctx.channel.send.await_count, 3)
...
class AdminSetHelperTests(HelperTestCase):
    ...
    async def test_rejects_non_positive_default_elo(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "default_elo must be greater than 0.", ephemeral=True
        )
...
class CurrentGameHelperTests(HelperTestCase):
    def _ctx(self):
        return FakeInteraction(self.guild, FakeMember("Caller"))

    async def test_defaults_to_league(self):
        ctx = self._ctx()
        await self.helperObj.currentGameHelper(ctx)
        ctx.response.send_message.assert_awaited_once_with("This server's current game is **League**.")

    async def test_reflects_a_switched_game(self):
        await self.helperObj.setGameHelper(self._ctx(), "Valorant")
        ctx = self._ctx()
        await self.helperObj.currentGameHelper(ctx)
        ctx.response.send_message.assert_awaited_once_with("This server's current game is **Valorant**.")

    async def test_notes_when_an_in_progress_roster_is_on_a_different_game(self):
        # servers.game (stamped when a roster forms) can diverge from
        # current_game once /set game runs again mid-game. See
        ...
        message = ctx.response.send_message.call_args.args[0]
        self.assertIn("current game is **League**", message)
        self.assertIn("in progress is still tracking **Valorant**", message)

    async def test_no_note_when_no_roster_has_diverged(self):
        ctx = self._ctx()
        await self.helperObj.currentGameHelper(ctx)
        message = ctx.response.send_message.call_args.args[0]
        self.assertNotIn("in progress", message)
...
class NotifyHelperTests(HelperTestCase):
    ...
    async def test_returns_true_when_the_dm_sends_successfully(self):
        voice_channel = FakeChannel("Lobby")
        caller = FakeMember("Caller")
        caller.voice = FakeVoiceState(voice_channel)
        ctx = FakeInteraction(self.guild, caller)
        ...
        result = await self.helperObj.notifyHelper(ctx, target)

        self.assertTrue(result)

    async def test_returns_false_when_the_member_has_dms_closed(self):
        voice_channel = FakeChannel("Lobby")
        caller = FakeMember("Caller")
        caller.voice = FakeVoiceState(voice_channel)
        ctx = FakeInteraction(self.guild, caller)
        target = FakeMember("Target")
        target.create_dm.return_value.send.side_effect = discord.Forbidden(
            SimpleNamespace(status=403, reason="Forbidden"), "Cannot send messages to this user"
        )
...
class EconomyTests(HelperTestCase):
    ...
    async def test_daily_second_claim_same_day_blocked(self):
        ...
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "balance"), 1000)
        ctx2.response.send_message.assert_awaited_once()
        args, kwargs = ctx2.response.send_message.call_args
        self.assertIn("<t:", args[0])
        self.assertIn(":R>", args[0])
        self.assertTrue(kwargs.get("ephemeral"))


class GiveGoldHelperTests(HelperTestCase):
    def _ctx(self, user_id=901, name="Alice"):
        return FakeInteraction(self.guild, FakeMember(name, id=user_id))

    async def test_rejects_giving_to_yourself(self):
        self.helperObj.ensureEconomyRow(GUILD_ID, 901, "Alice")
        ctx = self._ctx()
        await self.helperObj.giveGoldHelper(ctx, FakeMember("Alice", id=901), 100)
        ctx.response.send_message.assert_awaited_once_with(
            "You can't give gold to yourself!", ephemeral=True
        )

    async def test_rejects_giving_to_a_bot(self):
        ctx = self._ctx()
        target = FakeMember("SomeBot", id=902, bot=True)
        await self.helperObj.giveGoldHelper(ctx, target, 100)
        ctx.response.send_message.assert_awaited_once_with(
            "You can't give gold to a bot!", ephemeral=True
        )

    async def test_rejects_zero_amount(self):
        ctx = self._ctx()
        target = FakeMember("Bob", id=902)
        await self.helperObj.giveGoldHelper(ctx, target, 0)
        ctx.response.send_message.assert_awaited_once_with(
    ...
    async def test_rejects_negative_amount(self):
        ctx = self._ctx()
        target = FakeMember("Bob", id=902)
        await self.helperObj.giveGoldHelper(ctx, target, -50)
        ctx.response.send_message.assert_awaited_once_with(
            "Amount must be greater than 0.", ephemeral=True
        )
    ...
    async def test_rejects_insufficient_balance(self):
        self.helperObj.ensureEconomyRow(GUILD_ID, 901, "Alice")
        self.cursor.execute("UPDATE economy SET balance=50 WHERE guildId=? AND userId=?", (GUILD_ID, 901))
        self.db.commit()
        ctx = self._ctx()
        target = FakeMember("Bob", id=902)
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "You don't have enough gold for that! Your balance is 50.", ephemeral=True
        )
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "balance"), 50)

    async def test_successful_transfer_moves_gold_between_both_balances(self):
        self.helperObj.ensureEconomyRow(GUILD_ID, 901, "Alice")
        self.cursor.execute("UPDATE economy SET balance=1000 WHERE guildId=? AND userId=?", (GUILD_ID, 901))
        self.db.commit()
        ctx = self._ctx()
        ...
        await self.helperObj.giveGoldHelper(ctx, target, 300)

        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "balance"), 700)
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 902, "balance"), 300)
        message = ctx.response.send_message.call_args.args[0]
        self.assertIn("Alice", message)
        self.assertIn("Bob", message)
        self.assertIn("300", message)

    async def test_creates_an_economy_row_for_a_recipient_with_none_yet(self):
        self.helperObj.ensureEconomyRow(GUILD_ID, 901, "Alice")
        self.cursor.execute("UPDATE economy SET balance=1000 WHERE guildId=? AND userId=?", (GUILD_ID, 901))
        self.db.commit()
        ctx = self._ctx()
        ...
        self.cursor.execute("SELECT COUNT(*) FROM economy WHERE guildId=? AND userId=?", (GUILD_ID, 902))
        self.assertEqual(self.cursor.fetchone()[0], 1)

    async def test_transfer_does_not_touch_wins_losses_or_wager_columns(self):
        # A gift is even less bet-like than a cancelled duel (see
        # _finishDuelCancellation): nothing here was ever wagered.
        self.helperObj.ensureEconomyRow(GUILD_ID, 901, "Alice")
        self.cursor.execute("UPDATE economy SET balance=1000 WHERE guildId=? AND userId=?", (GUILD_ID, 901))
        self.db.commit()
        ctx = self._ctx()
        ...
        for user_id in (901, 902):
            self.assertEqual(self.helperObj.getEconomy(GUILD_ID, user_id, "wins"), 0)
            self.assertEqual(self.helperObj.getEconomy(GUILD_ID, user_id, "losses"), 0)
            self.assertEqual(self.helperObj.getEconomy(GUILD_ID, user_id, "gold_wagered"), 0)
            self.assertEqual(self.helperObj.getEconomy(GUILD_ID, user_id, "gold_won"), 0)
            self.assertEqual(self.helperObj.getEconomy(GUILD_ID, user_id, "gold_lost"), 0)
...
class WagerHelperTests(HelperTestCase):
    ...
    async def test_allows_amount_at_exactly_the_max_wager_cap(self):
        ...
        await self.helperObj.wagerHelper(ctx, 100, 1)
        ctx.response.send_message.assert_awaited_once()
        self.assertEqual(ctx.response.send_message.call_args.args[0], "You wagered 100 gold on **Team 1**!")
    ...
    async def test_no_cap_set_allows_any_amount(self):
        ...
        ctx.response.send_message.assert_awaited_once()
        self.assertEqual(ctx.response.send_message.call_args.args[0], "You wagered 50000 gold on **Team 1**!")
    ...
    async def test_rejects_when_betting_not_open(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            ...
            "and open betting.", ephemeral=True
        )
    ...
    async def test_rejects_insufficient_balance(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "You don't have enough gold for that! Your balance is 0.", ephemeral=True
        )
    ...
    async def test_rejects_duplicate_bet(self):
        ...
        ctx2.response.send_message.assert_awaited_once_with(
            "You've already placed a bet on this game.", ephemeral=True
        )
    ...
    async def test_successful_wager_escrows_gold(self):
        ...
        ctx.response.send_message.assert_awaited_once()
        self.assertEqual(ctx.response.send_message.call_args.args[0], "You wagered 250 gold on **Team 2**!")
    ...
    async def test_rejects_wager_from_a_player_in_the_game(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "You can't wager on a game you're playing in!", ephemeral=True
        )
    ...
    async def _place_bet(self, user_id=901, name="Alice", amount=250, team=1):
        self.helperObj.update(GUILD_ID, "betting_state", "OPEN")
        self.helperObj.ensureEconomyRow(GUILD_ID, user_id, name)
        self.cursor.execute(
            "UPDATE economy SET balance=1000 WHERE guildId=? AND userId=?", (GUILD_ID, user_id)
        ...
        self.db.commit()
        ctx = self._ctx(user_id, name)
        await self.helperObj.wagerHelper(ctx, amount, team)
        return ctx

    async def test_confirmation_attaches_a_cancel_view(self):
        ctx = await self._place_bet()
        view = ctx.response.send_message.call_args.kwargs["view"]
        self.assertIsInstance(view, helper_module.WagerCancelView)
        self.assertIsNone(view.match_id)

    async def test_bettor_cancelling_refunds_and_removes_the_wager(self):
        await self._place_bet(amount=250)
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "balance"), 750)

        click = self._ctx()
        await self.helperObj._handleWagerCancelClick(click, GUILD_ID, 901, None)

        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "balance"), 1000)
        self.cursor.execute("SELECT COUNT(*) FROM wagers WHERE guildId=? AND userId=?", (GUILD_ID, 901))
        self.assertEqual(self.cursor.fetchone()[0], 0)
        click.response.edit_message.assert_awaited_once()
        self.assertIn("refunded 250", click.response.edit_message.call_args.kwargs["content"])

    async def test_someone_else_cannot_cancel_another_players_bet(self):
        await self._place_bet(amount=250)
        click = self._ctx(user_id=902, name="Mallory")
        await self.helperObj._handleWagerCancelClick(click, GUILD_ID, 901, None)
    ...
    async def test_cannot_cancel_once_betting_has_closed(self):
        await self._place_bet(amount=250)
        ...
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "balance"), 750)
        self.assertIn("already closed", click.response.send_message.call_args.args[0])

    async def test_cancelling_twice_the_second_click_is_rejected_without_double_refunding(self):
        await self._place_bet(amount=250)
        await self.helperObj._handleWagerCancelClick(self._ctx(), GUILD_ID, 901, None)

        click2 = self._ctx()
        await self.helperObj._handleWagerCancelClick(click2, GUILD_ID, 901, None)

        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "balance"), 1000)
        self.assertIn("already been cancelled", click2.response.send_message.call_args.args[0])
...
class RecordResultTests(HelperTestCase):
    ...
    async def test_pari_mutuel_uneven_pools_favor_underdog(self):
        ...
        # _announceAchievements). This scans every call instead of
        # assuming this is the final one, the same fix the First Blood
        # case needed.
    ...
    async def test_a_heavily_favored_win_is_raked_below_the_full_pari_mutuel_split(self):
        ...
        # favorite (300 vs 100, a 0.75 winning share). That's exactly the
        # "safe bettor" case _imbalanceRakeFraction exists to tax.
    ...
    async def test_casual_game_with_roster_does_not_touch_elo(self):
        # Regression test: /make-teams random and /make-teams draft form
        # rosters just like ranked:true does, but is_ranked defaults to 0
        # for them. Elo must stay untouched even though
        # game_wins/game_losses still update.
        team1 = Team(); team1.name = "Team 1"
        team1.add_player(Player(701, "P1"))
        team2 = Team(); team2.name = "Team 2"
        team2.add_player(Player(702, "P2"))
    ...
    async def test_rostered_players_get_win_or_loss_gold_accordingly(self):
        ...
        messages = [c.args[0] for c in channel.send.call_args_list]
        self.assertTrue(any(
            f"1 winning player earned {helper_module.GAME_WIN_GOLD} gold and "
            f"1 losing player earned {helper_module.GAME_LOSS_GOLD} gold just for playing." in m
            for m in messages
        ))
    ...
    async def test_does_not_touch_make_teams_messages_for_a_tournament_match(self):
        ...
        # _handleReadyClick), so roster_team1_message_id here would just
        # be a stale leftover from some earlier, unrelated casual game.
    ...
    async def test_does_not_reply_to_a_stale_matchup_graphic_for_a_tournament_match(self):
        ...
        # game never wins out. It's left untouched, since
...
class ComputeGameDeltasTests(HelperTestCase):
    ...
    def test_participation_gold_stacks_with_bet_payouts_without_touching_wager_fields(self):
        # user 1 is rostered AND bet on their own game... no, betting on a
        # game you're playing in is blocked elsewhere. This covers a
        # rostered player (1) and a pure bettor (901) independently,
        # confirming participation gold only ever lands via the roster
        # loop and never perturbs gold_wagered/gold_won/gold_lost either
        # way.
        deltas, _summary = self.helperObj.computeGameDeltas(
...
class FormatResultMessageTests(HelperTestCase):
    ...
    def test_falls_back_to_a_generic_label_when_summary_has_no_names(self):
        # A caller that hasn't threaded real names through
        # computeGameDeltas (or an old-style summary dict) shouldn't
        # KeyError.
    ...
    def test_announces_win_gold_only_when_there_are_no_losers(self):
        ...
        message = self.helperObj.formatResultMessage(1, summary)
        self.assertIn(
            f"1 winning player earned {helper_module.GAME_WIN_GOLD} gold just for playing.", message
        )

    def test_omits_each_only_for_the_side_with_exactly_one_player(self):
        # A lone winner against a multi-player losing side isn't a full
        # 1v1, but "1 winning player earned N gold each" still reads wrong
        # for that one player specifically. Each side's wording is judged
        # independently rather than only checking if the whole game was
        # 1v1.
        _deltas, summary = self.helperObj.computeGameDeltas(
            wagers=[], team1_roster=[(1, "A")], team2_roster=[(2, "B")],
            elo_lookup={1: 1000, 2: 1000}, winning_team=1,
        )
        message = self.helperObj.formatResultMessage(1, summary)
        self.assertIn(
            f"1 winning player earned {helper_module.GAME_WIN_GOLD} gold and "
            f"1 losing player earned {helper_module.GAME_LOSS_GOLD} gold just for playing.",
            message,
        )
...
class ReportCorrectWinnerHelperTests(HelperTestCase):
    async def test_no_recent_result_to_correct(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "There's no recent game result to correct.", ephemeral=True
        )
    ...
    async def test_already_recorded_winner_is_a_noop(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "**Team 1** is already the recorded winner; nothing to correct.", ephemeral=True
        )
    ...
    async def test_corrects_bettor_payouts_when_winner_flips(self):
        ...
        view = ctx.response.send_message.call_args.kwargs["view"]
        click = FakeInteraction(self.guild, FakeMember("Admin"))
        await view.confirm.callback(click)
        ...
        # It's not conserved, and that's the rake doing its job.
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "balance"), 900)
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "wins"), 0)
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "losses"), 1)
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "gold_won"), 0)
        ...
        result_text = click.response.edit_message.call_args.kwargs["content"]
        self.assertIn("Team 2", result_text)
        self.assertIn("previously recorded as Team 1", result_text)
    ...
    async def test_correction_still_credits_the_disliked_role_bonus(self):
        ...
        view = ctx.response.send_message.call_args.kwargs["view"]
        click = FakeInteraction(self.guild, FakeMember("Admin"), channel=ctx.channel)
        await view.confirm.callback(click)

        # Back to 1000 (the wrong loss reversed) plus the disliked-role
        # bonus for the real win. This is recomputed from the last_result
        ...
        # message rather than assuming the result message is the final
        # one.
        messages = [c.args[0] for c in ctx.channel.send.call_args_list if c.args]
        self.assertTrue(any("Disliked-role win bonus: P1" in m for m in messages))
    ...
    async def test_invalidate_rejects_when_team_also_given(self):
        ...
        ctx.response.send_message.assert_awaited_once_with("Give team or invalidate, not both.", ephemeral=True)
    ...
    async def test_invalidate_rejects_a_specific_tournament_match(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            ...
            "to the other team instead.", ephemeral=True
        )
    ...
    async def test_invalidate_refunds_bets_and_undoes_elo_and_records(self):
        ...
        result_text = click.response.edit_message.call_args.kwargs["content"]
        self.assertIn("Team 1", result_text)
        self.assertIn("Team 2", result_text)
        self.assertIn("invalidated", result_text)

        # nothing left to correct further: the last result is gone entirely
        self.assertIsNone(self.helperObj.getLastResult(GUILD_ID))

    # Regression: /set correct-winner's last-game path (correcting or
    # invalidating) used to reverse and reapply real payouts/elo/records
    # the instant the command ran, no confirmation at all. Same
    # one-accidental-click risk every other winner-report flow in the bot
    # already guards against.
    async def test_cancelling_leaves_the_correction_unapplied(self):
        self.helperObj.saveLastResult(GUILD_ID, winning_team=1, wagers=[], team1_roster=[], team2_roster=[], deltas={})
        ctx = FakeInteraction(self.guild, FakeMember("Admin"))
        await self.helperObj.reportCorrectWinnerHelper(ctx, 2)
        view = ctx.response.send_message.call_args.kwargs["view"]
        ...
        self.assertEqual(self.helperObj.getLastResult(GUILD_ID)["winning_team"], 1)
        self.assertIn("Nothing was corrected", click.response.edit_message.call_args.kwargs["content"])
    ...
    async def test_confirming_after_a_newer_game_resolved_refuses_instead_of_applying(self):
        # The prompt was built from a snapshot of the last game. If a
        # newer game resolves (replacing last_result) before Confirm is
        # pressed, applying the stale snapshot's deltas would corrupt the
        # newer game's own numbers, so this refuses instead.
        self.helperObj.saveLastResult(GUILD_ID, winning_team=1, wagers=[], team1_roster=[], team2_roster=[], deltas={})
        ctx = FakeInteraction(self.guild, FakeMember("Admin"))
        await self.helperObj.reportCorrectWinnerHelper(ctx, 2)
        view = ctx.response.send_message.call_args.kwargs["view"]

        # a different, newer game resolves in the gap
        self.helperObj.saveLastResult(GUILD_ID, winning_team=2, wagers=[], team1_roster=[], team2_roster=[], deltas={})

        click = FakeInteraction(self.guild, FakeMember("Admin"))
        await view.confirm.callback(click)

        # the newer game's own result is untouched, not overwritten by the
        # stale correction
        self.assertEqual(self.helperObj.getLastResult(GUILD_ID)["winning_team"], 2)
        self.assertIn("changed", click.response.edit_message.call_args.kwargs["content"])
...
class StatsHelperTests(HelperTestCase):
    ...
    async def test_fields_are_grouped_ranked_then_casual_then_gold(self):
        ...
        # the line break after the ranked row, but it still rendered its
...
class StatsViewTests(HelperTestCase):
    ...
    async def test_toggle_noops_if_the_member_left_the_guild(self):
        # Alice deliberately NOT added to self.guild.members, simulating
    ...
    async def test_avatar_toggle_on_the_card_swaps_in_the_global_avatar(self):
        ...
        # FakeClient.fetch_user). It's resolved and passed straight through,
...
class AchievementsHelperTests(HelperTestCase):
    ...
    async def test_other_field_stays_within_discords_1024_char_limit(self):
        ctx = self._ctx()
        await self.helperObj.achievementsHelper(ctx)
        embed = ctx.response.send_message.call_args.kwargs["embed"]
        other_field = next(field for field in embed.fields if field.name == "__Other__")
        self.assertLessEqual(len(other_field.value), 1024)

    def test_join_embed_field_lines_keeps_everything_under_the_limit(self):
        lines = ["a short line"] * 5
        joined = self.helperObj._joinEmbedFieldLines(lines, limit=1024)
        self.assertEqual(joined, "\n".join(lines))

    def test_join_embed_field_lines_truncates_and_notes_how_many_were_cut(self):
        lines = [f"line {i} " + "x" * 20 for i in range(50)]
        joined = self.helperObj._joinEmbedFieldLines(lines, limit=200)

        self.assertLessEqual(len(joined), 200)
        self.assertIn("more", joined)
        self.assertTrue(joined.startswith("line 0 "))

    async def test_locked_achievement_shows_its_progress_in_the_embed(self):
        self.helperObj.ensureEconomyRow(GUILD_ID, 901, "Alice")
        self.helperObj.ensureGameStatsRow(GUILD_ID, 901, "Alice", "League")
        self.cursor.execute(
            "UPDATE game_stats SET game_wins=3 WHERE guildId=? AND userId=? AND game='League'", (GUILD_ID, 901)
        )
        self.db.commit()
        ...
        embed = ctx.response.send_message.call_args.kwargs["embed"]
        veteran_field = next(field for field in embed.fields if field.name == "__Veteran__")
        self.assertIn(f"(3/{helper_module.CARD_ACHIEVEMENT_VETERAN_WINS})", veteran_field.value)

    async def test_earned_achievement_shows_no_progress_fraction(self):
        self.helperObj.ensureEconomyRow(GUILD_ID, 901, "Alice")
        self.helperObj.ensureGameStatsRow(GUILD_ID, 901, "Alice", "League")
        self.cursor.execute(
            "UPDATE game_stats SET game_wins=1 WHERE guildId=? AND userId=? AND game='League'", (GUILD_ID, 901)
        ...
        embed = ctx.response.send_message.call_args.kwargs["embed"]
        other_field = next(field for field in embed.fields if field.name == "__Other__")
        first_blood_line = next(
            line for line in other_field.value.split("\n")
            if helper_module.CARD_ACHIEVEMENT_TITLES["first_blood"] in line
        )
        self.assertNotIn("(", first_blood_line)


class AchievementProgressTests(HelperTestCase):
    def _catalog(self, user_id=901):
        return {item["key"]: item for item in self.helperObj.getAchievementCatalog(GUILD_ID, user_id)}

    async def test_veteran_ladder_progress_tracks_game_wins(self):
        self.helperObj.ensureEconomyRow(GUILD_ID, 901, "Alice")
        self.helperObj.ensureGameStatsRow(GUILD_ID, 901, "Alice", "League")
        self.cursor.execute(
            "UPDATE game_stats SET game_wins=3 WHERE guildId=? AND userId=? AND game='League'", (GUILD_ID, 901)
        ...
        self.assertEqual(catalog["veteran"]["progress"], (3, helper_module.CARD_ACHIEVEMENT_VETERAN_WINS))
        self.assertFalse(catalog["veteran"]["earned"])

    async def test_on_fire_ladder_progress_tracks_current_win_streak(self):
        self.helperObj.ensureEconomyRow(GUILD_ID, 901, "Alice")
        self.helperObj.ensureGameStatsRow(GUILD_ID, 901, "Alice", "League")
        self.cursor.execute(
            "UPDATE game_stats SET current_win_streak=2 WHERE guildId=? AND userId=? AND game='League'",
            (GUILD_ID, 901),
        )
        self.db.commit()

        catalog = self._catalog()
        self.assertEqual(catalog["on_fire"]["progress"], (2, helper_module.CARD_ACHIEVEMENT_ON_FIRE_STREAK))

    async def test_gambler_progress_counts_bet_wins_and_losses_together(self):
        self.helperObj.ensureEconomyRow(GUILD_ID, 901, "Alice")
        self.cursor.execute("UPDATE economy SET wins=4, losses=6 WHERE guildId=? AND userId=?", (GUILD_ID, 901))
        self.db.commit()

        catalog = self._catalog()
        self.assertEqual(catalog["gambler"]["progress"], (10, helper_module.CARD_ACHIEVEMENT_GAMBLER_BETS))

    async def test_iron_will_progress_tracks_game_losses(self):
        self.helperObj.ensureEconomyRow(GUILD_ID, 901, "Alice")
        self.helperObj.ensureGameStatsRow(GUILD_ID, 901, "Alice", "League")
        self.cursor.execute(
            "UPDATE game_stats SET game_losses=5 WHERE guildId=? AND userId=? AND game='League'", (GUILD_ID, 901)
        )
        self.db.commit()

        catalog = self._catalog()
        self.assertEqual(catalog["iron_will"]["progress"], (5, helper_module.CARD_ACHIEVEMENT_IRON_WILL_LOSSES))

    async def test_team_player_progress_counts_teams_rostered_on(self):
        for name in ("Red", "Blue"):
            ctx = FakeInteraction(self.guild, FakeMember("Alice", id=901))
            await self.helperObj.createTeamHelper(ctx, name, 5)

        catalog = self._catalog()
        self.assertEqual(catalog["team_player"]["progress"], (2, helper_module.CARD_ACHIEVEMENT_TEAM_PLAYER_TEAMS))

    async def test_binary_achievements_have_no_progress_fraction(self):
        catalog = self._catalog()
        self.assertIsNone(catalog["captain"]["progress"])
        self.assertIsNone(catalog["first_blood"]["progress"])
        self.assertIsNone(catalog["tournament_champion"]["progress"])

    async def test_earned_achievement_has_no_progress_fraction(self):
        self.helperObj.ensureEconomyRow(GUILD_ID, 901, "Alice")
        self.helperObj._unlockAchievement(GUILD_ID, 901, "first_blood")

        catalog = self._catalog()
        self.assertTrue(catalog["first_blood"]["earned"])
        self.assertIsNone(catalog["first_blood"]["progress"])

    async def test_progress_is_scoped_to_the_currently_tracked_game(self):
        self.helperObj.ensureEconomyRow(GUILD_ID, 901, "Alice")
        self.helperObj.ensureGameStatsRow(GUILD_ID, 901, "Alice", "League")
        self.helperObj.ensureGameStatsRow(GUILD_ID, 901, "Alice", "Valorant")
        self.cursor.execute(
            "UPDATE game_stats SET game_wins=7 WHERE guildId=? AND userId=? AND game='Valorant'", (GUILD_ID, 901)
        )
        self.db.commit()

        # Still tracking League by default, so Valorant's 7 wins shouldn't
        # show up as this player's veteran progress here.
        catalog = self._catalog()
        self.assertEqual(catalog["veteran"]["progress"], (0, helper_module.CARD_ACHIEVEMENT_VETERAN_WINS))
...
class CardUnlocksTests(HelperTestCase):
    ...
    def test_reaching_a_higher_tier_unlocks_every_lower_tier_reward_too(self):
        ...
        # Master along the way, not just the top tier actually landed
        # on.
    ...
    def test_color_scheme_background_stays_the_raw_darkened_badge_color(self):
        ...
        # still the badge color's own straight 28%-darkened shade, the
        # same as the team card's own derivation, so a scheme's overall
        # mood still authentically reflects the tier that earned it.
        self.helperObj._checkTierRewardUnlocks(GUILD_ID, 901, helper_module.ELO_TIER_THRESHOLDS["Diamond"])
        schemes = {s["name"]: s for s in self.helperObj.getUnlockedCardColorSchemes(GUILD_ID, 901)}
...
class CardSetHelperTests(HelperTestCase):
    ...
    async def test_equipping_the_default_title_always_succeeds(self):
        ...
        # An immediate placeholder ack, then the real card via an edit.
        # See cardSetHelper's own defer/edit comment.
        ctx.response.send_message.assert_awaited_once()
    ...
    async def test_equipped_scheme_tracks_later_changes_to_its_catalog_entry(self):
        # A player who equips, say, "Fire" shouldn't have its colors
        # frozen forever at whatever they were computed to be the moment
        ...
        # (or to CARD_MIN_ACCENT_CONTRAST itself). That's the same
        # staleness the customized flag protects the default palette from.
        self.cursor.execute(
...
class ShopTests(HelperTestCase):
    ...
    def test_every_title_and_font_name_is_globally_unique(self):
        ...
        # reused as the CARD_DEFAULT_TITLE/CARD_DEFAULT_FONT_STYLE
        # everyone already has for free) would make one shadow the other.
        # Every title-ish and font-ish name in the game has to be
        # pairwise distinct for that to stay safe.
        title_like_sources = [
...
class TradingCardSettingsTests(HelperTestCase):
    ...
    def test_ensure_resyncs_an_uncustomized_row_to_current_defaults(self):
        # Since there's no /card-customize command yet, an uncustomized
        # row is always just a stale snapshot, not a real choice. It
        # should track the current CARD_DEFAULT_* values instead of
        # staying frozen at whatever they were when the row was first
        # created.
        self.helperObj.ensureCardSettings(GUILD_ID, 901)
        self.cursor.execute(
...
class RenderTradingCardImageTests(HelperTestCase):
    ...
    def test_name_font_shrinks_to_fit_a_long_username_in_every_shop_font(self):
        # Regression test: PRESS_START_2P ("Retro")'s near-monospace
        # glyphs are unusually wide. A real (up to 32-char) Discord
        # username at the standard CARD_NAME_FONT_SIZE could measure at or
        # past CARD_WIDTH itself, well past the card's own border, where
        # every other bundled font stays comfortably clear of the edge.
    ...
    def test_name_font_never_shrinks_below_the_floor(self):
        ...
        # human could read. _fitNameFont has to stop somewhere rather
        # than shrinking toward 0.
    ...
    def test_caps_team_rows_and_shows_a_remainder_count(self):
        ...
        # reaction-flow tests (HandleStatsReactionTests), since checking
        # actual pixel/text content here is harder without an OCR step.
    ...
    def test_custom_colors_are_respected(self):
        ...
        # pixel color. That's the simplest observable proof the setting
        # actually reached the renderer instead of silently falling back
        # to default.
        settings = {
    ...
    def test_font_style_changes_body_text_not_just_the_name_and_title(self):
        ...
        # values, roster rows, username), not just name_font/title_font.
        # A stat label (drawn with label_font) is the cheapest one to
        # compare pixel-for-pixel between styles.
    ...
    def test_every_font_style_uses_its_own_distinct_font_file(self):
        # Not just distinct from the default style. Also distinct from
        # every OTHER shop style, so two styles can't accidentally end up
    ...
    def test_every_shop_font_file_actually_loads(self):
        ...
        # (OSError/ValueError caught and swallowed). That's real handling
        # for a genuinely missing/renamed style, but it means a corrupted
        # or truncated font FILE wouldn't be caught by any test that only
        # goes through _loadFont. Loading each bundled shop-font file
        # directly instead makes a bad download or corrupted TTF fail
        # loudly here.
        for font_style in helper_module.CARD_SHOP_FONT_STYLES:
...
class OpenBettingTests(HelperTestCase):
    async def test_opens_betting_and_schedules_timer(self):
        ...
        # it finishes *normally* rather than raising. That's fine: what
...
class GetBettingTimerSecondsTests(HelperTestCase):
    ...
    def test_falls_back_to_the_default_when_the_guild_has_no_row_at_all(self):
        ...
        # (/test's simulated tournament is one such case), shouldn't
        # crash just because it wants a betting duration.
        self.assertEqual(
...
class WagerTeamNameResolutionTests(HelperTestCase):
    def test_get_wager_team_names_falls_back_without_a_current_game(self):
        self.assertEqual(self.helperObj.getWagerTeamNames(GUILD_ID), ("Team 1", "Team 2"))

    def test_get_wager_team_names_reads_the_current_roster(self):
        team1, team2 = Team(), Team()
        team1.set_name("Ravens")
        team2.set_name("Falcons")
        self.helperObj.update(GUILD_ID, "team1", team1.serializeTeam())
        self.helperObj.update(GUILD_ID, "team2", team2.serializeTeam())

        self.assertEqual(self.helperObj.getWagerTeamNames(GUILD_ID), ("Ravens", "Falcons"))

    def test_get_wager_team_names_reads_a_specific_match(self):
        team1, team2 = Team(), Team()
        team1.set_name("Ravens")
        team2.set_name("Falcons")
        self.cursor.execute(
            "INSERT INTO tournament_matches(id, guildId, roundIndex, nodeIndex, team1, team2, state, "
            "mode, messageId, channelId, winner, bracketType) "
            "VALUES(5, ?, 0, 0, ?, ?, 'AWAITING_RESULT', 'simultaneous', NULL, NULL, NULL, 'winners')",
            (GUILD_ID, team1.serializeTeam(), team2.serializeTeam())
        )
        self.db.commit()

        self.assertEqual(self.helperObj.getWagerTeamNames(GUILD_ID, match_id=5), ("Ravens", "Falcons"))

    def test_get_wager_team_names_falls_back_for_an_unknown_match(self):
        self.assertEqual(self.helperObj.getWagerTeamNames(GUILD_ID, match_id=999), ("Team 1", "Team 2"))

    def test_resolve_accepts_numeric_strings(self):
        self.assertEqual(self.helperObj.resolveWagerTeamValue("1", "Ravens", "Falcons"), 1)
        self.assertEqual(self.helperObj.resolveWagerTeamValue("2", "Ravens", "Falcons"), 2)

    def test_resolve_accepts_real_names_case_insensitively(self):
        self.assertEqual(self.helperObj.resolveWagerTeamValue("ravens", "Ravens", "Falcons"), 1)
        self.assertEqual(self.helperObj.resolveWagerTeamValue("FALCONS", "Ravens", "Falcons"), 2)

    def test_resolve_returns_none_for_anything_else(self):
        self.assertIsNone(self.helperObj.resolveWagerTeamValue("Seahawks", "Ravens", "Falcons"))
...
class PlaceTournamentWagerTests(HelperTestCase):
    ...
    async def test_rejects_unknown_match_id(self):
        ...
        ctx.response.send_message.assert_awaited_once_with(
            "No tournament match with id 42 in this server.", ephemeral=True
        )
    ...
    async def test_rejects_when_match_is_resolved(self):
        ...
        ctx.response.send_message.assert_awaited_once_with("Betting is closed for match #1.", ephemeral=True)
    ...
    async def test_rejects_once_a_report_reaction_is_pending_confirmation(self):
        ...
        # the match immediately, but the confirmation step now leaves it
        # in a CONFIRMING state for a bit. Betting has to close right at
        # the reaction, not just once Confirm is actually pressed, or
        # someone could still bet on the reported side during that
        # window.
    ...
    async def test_rejects_a_rostered_player(self):
        ...
        await self.helperObj.wagerHelper(ctx, 100, 1, match_id=1)
        ctx.response.send_message.assert_awaited_once_with(
            "You can't wager on a match you're playing in!", ephemeral=True
        )
    ...
    async def test_rejects_duplicate_bet_on_the_same_match(self):
        ...
        ctx2.response.send_message.assert_awaited_once_with(
            "You've already placed a bet on match #1.", ephemeral=True
        )
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "balance"), 900)
    ...
    async def test_successful_wager_escrows_gold(self):
        ...
        ctx.response.send_message.assert_awaited_once()
        self.assertEqual(
            ctx.response.send_message.call_args.args[0], "You wagered 250 gold on **Team 2** for match #1!"
        )

    async def test_confirmation_uses_the_matchs_own_real_team_name(self):
        team1, team2 = Team(), Team()
        team1.name = "Ravens"
        team2.name = "Falcons"
        self._insert_match(2, team1=team1, team2=team2)
        self._give_gold(901, "Alice", 1000)

        ctx = self._ctx()
        await self.helperObj.wagerHelper(ctx, 250, 2, match_id=2)

        ctx.response.send_message.assert_awaited_once()
        self.assertEqual(
            ctx.response.send_message.call_args.args[0], "You wagered 250 gold on **Falcons** for match #2!"
        )
    ...
    async def test_confirmation_attaches_a_cancel_view_scoped_to_the_match(self):
        self._insert_match(1)
        self._give_gold(901, "Alice", 1000)
        ctx = self._ctx()
        await self.helperObj.wagerHelper(ctx, 250, 1, match_id=1)

        view = ctx.response.send_message.call_args.kwargs["view"]
        self.assertIsInstance(view, helper_module.WagerCancelView)
        self.assertEqual(view.match_id, 1)

    async def test_bettor_cancelling_refunds_and_removes_the_tournament_wager(self):
        self._insert_match(1)
        self._give_gold(901, "Alice", 1000)
        await self.helperObj.wagerHelper(self._ctx(), 250, 1, match_id=1)
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "balance"), 750)

        click = self._ctx()
        await self.helperObj._handleWagerCancelClick(click, GUILD_ID, 901, 1)

        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "balance"), 1000)
        self.cursor.execute("SELECT COUNT(*) FROM tournament_wagers WHERE matchId=? AND userId=?", (1, 901))
        self.assertEqual(self.cursor.fetchone()[0], 0)
        click.response.edit_message.assert_awaited_once()

    async def test_cannot_cancel_once_betting_is_closed_for_the_match(self):
        self._insert_match(1)
        self._give_gold(901, "Alice", 1000)
        await self.helperObj.wagerHelper(self._ctx(), 250, 1, match_id=1)
        self.cursor.execute("UPDATE tournament_matches SET bettingClosed=1 WHERE id=1")
        self.db.commit()
        ...
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "balance"), 750)
        self.assertIn("closed for match #1", click.response.send_message.call_args.args[0])
...
class SettleMatchWagersTests(HelperTestCase):
    def _bet(self, match_id, user_id, name, team, amount):
        ...
        # row plus an economy row. The real wagerHelper flow debits
        # `amount` from balance up front, and settlement only ever
        # credits winnings back. So starting from balance=0 lets a
        # settled balance be compared directly against the expected
        # payout.
        self.helperObj.ensureEconomyRow(GUILD_ID, user_id, name)
        self.cursor.execute(
...
class WinnerReportViewTests(HelperTestCase):
    ...
    async def test_valid_click_clears_message_id_and_posts_a_confirmation_instead_of_recording(self):
        ...
        # A real elo/payout change shouldn't hinge on the click alone.
        # It's not recorded until the posted confirmation is actually clicked.
    ...
    # Regression coverage for the "only admins and rostered players can
    # report/cancel" gate: a bystander (not on either team, not an admin)
    # must be rejected before anything about the game/betting state is
    # even touched.
    async def test_report_click_rejects_a_bystander(self):
        click = self._click(user_id=999)
        click.user.guild_permissions.manage_guild = False
        ...
        self.assertNotIn("reported as the winner", click.response.send_message.call_args.args[0])
        self.assertTrue(click.response.send_message.call_args.kwargs.get("ephemeral"))
        # never touched, unlike the valid-click path
        self.assertEqual(self.helperObj.get(GUILD_ID, "betting_message_id"), 555)

    async def test_report_click_allows_a_rostered_non_admin_player(self):
        team1 = Team()
        team1.add_player(Player(999, "Rostered"))
        self.helperObj.update(GUILD_ID, "team1", team1.serializeTeam())
        click = self._click(user_id=999)
        click.user.guild_permissions.manage_guild = False
    ...
    async def test_cancel_click_rejects_a_bystander(self):
        click = self._click(user_id=999)
        click.user.guild_permissions.manage_guild = False
        with patch.object(self.helperObj, "cancelGameHelper", AsyncMock()) as cancel_mock:
            await helper_module.WinnerReportView(self.helperObj).cancelGame.callback(click)
...
class ConfirmWinnerReportViewTests(HelperTestCase):
    ...
    async def test_confirm_records_the_result_and_deletes_both_messages(self):
        ...
        # actually recorded. Neither lingers once formatResultMessage's
        # own message (posted by recordResult) has said the same thing
        # for good.
    ...
    async def test_cancel_does_not_restore_if_the_game_was_already_resolved_another_way(self):
        ...
        # clearTeamsHelper did) while this confirmation was still
        # pending. betting_state is no longer OPEN/CLOSED, so restoring
        # a stale betting_message_id here would resurrect an
        # already-settled game.
        self.helperObj.update(GUILD_ID, "betting_state", "NONE")
        self.helperObj.update(GUILD_ID, "betting_message_id", None)
        view = helper_module.ConfirmWinnerReportView(self.helperObj, GUILD_ID, 1, 555)
    ...
    # discord.py calls interaction_check before ever routing to a button's
    # own callback (the callback tests above call .confirm.callback(...)
    # directly, which bypasses it entirely), so this is the only place
    # that actually exercises the "admin or rostered player" gate itself.
    async def test_interaction_check_rejects_a_bystander(self):
        view = helper_module.ConfirmWinnerReportView(self.helperObj, GUILD_ID, 1, 555)
        click = self._click(user_id=999)
        click.user.guild_permissions.manage_guild = False
    ...
    async def test_interaction_check_allows_a_rostered_non_admin_player(self):
        team2 = Team()
        team2.add_player(Player(999, "Rostered"))
        self.helperObj.update(GUILD_ID, "team2", team2.serializeTeam())
        view = helper_module.ConfirmWinnerReportView(self.helperObj, GUILD_ID, 1, 555)
        click = self._click(user_id=999)
        click.user.guild_permissions.manage_guild = False
    ...
    async def test_interaction_check_allows_an_admin_who_is_not_rostered(self):
        view = helper_module.ConfirmWinnerReportView(self.helperObj, GUILD_ID, 1, 555)
        click = self._click(user_id=999)  # manage_guild=True by default
...
class ChallengeDuelHelperTests(HelperTestCase):
    ...
    async def test_rejects_challenging_yourself(self):
        ...
        ctx.response.send_message.assert_awaited_once_with("You can't wager against yourself!", ephemeral=True)

    async def test_rejects_challenging_a_bot(self):
        ctx = self._ctx()
        target = FakeMember("Botty", id=902, bot=True)
        await self.helperObj.challengeDuelHelper(ctx, target, 100)
        ctx.response.send_message.assert_awaited_once_with("You can't wager against a bot!", ephemeral=True)
    ...
    async def test_rejects_a_second_challenge_against_the_same_pending_target(self):
        self.helperObj.ensureEconomyRow(GUILD_ID, 901, "Alice")
        self.cursor.execute(
            "UPDATE economy SET balance=1000 WHERE guildId=? AND userId=?", (GUILD_ID, 901)
        )
        ...
        first = self._ctx()
        first.original_response.return_value = FakeMessage(id=1)
        await self.helperObj.challengeDuelHelper(first, target, 100)

        second = self._ctx()
        await self.helperObj.challengeDuelHelper(second, target, 50)

        second.response.send_message.assert_awaited_once()
        message = second.response.send_message.call_args.args[0]
        self.assertIn("already have a pending challenge", message)
        self.assertIn(target.mention, message)
        self.cursor.execute("SELECT COUNT(*) FROM duels")
        self.assertEqual(self.cursor.fetchone()[0], 1)
        # No gold moved for the rejected second attempt.
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "balance"), 1000)

    async def test_a_new_challenge_against_a_different_target_is_unaffected(self):
        self.helperObj.ensureEconomyRow(GUILD_ID, 901, "Alice")
        self.cursor.execute(
            "UPDATE economy SET balance=1000 WHERE guildId=? AND userId=?", (GUILD_ID, 901)
        )
        ...
        first = self._ctx()
        first.original_response.return_value = FakeMessage(id=1)
        await self.helperObj.challengeDuelHelper(first, FakeMember("Bob", id=902), 100)

        second = self._ctx()
        second.original_response.return_value = FakeMessage(id=2)
        await self.helperObj.challengeDuelHelper(second, FakeMember("Cleo", id=903), 50)

        second.response.send_message.assert_awaited_once()
        self.assertIsInstance(second.response.send_message.call_args.kwargs["view"], helper_module.DuelAcceptView)
        self.cursor.execute("SELECT COUNT(*) FROM duels")
        self.assertEqual(self.cursor.fetchone()[0], 2)
...
class DuelAcceptViewTests(HelperTestCase):
    ...
    # Regression: a pending challenge used to have no way to make it go
    # away short of the target just accepting it or leaving it sitting
    # there forever. See DuelAcceptView's own comment.
    async def test_decline_from_target_deletes_the_pending_duel(self):
        click = self._click(555, 902)
        await helper_module.DuelAcceptView(self.helperObj).decline.callback(click)

        self.cursor.execute("SELECT COUNT(*) FROM duels WHERE messageId=555")
        self.assertEqual(self.cursor.fetchone()[0], 0)
        # nothing was ever escrowed, so nothing to refund on a decline
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "balance"), 1000)
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 902, "balance"), 1000)
        click.response.send_message.assert_awaited_once_with("**Bob** declined **Alice**'s wager.")

    async def test_decline_from_challenger_is_rejected(self):
        click = self._click(555, 901)
        await helper_module.DuelAcceptView(self.helperObj).decline.callback(click)
    ...
    async def test_decline_ignores_mismatched_message_id(self):
        click = self._click(999, 902)
        await helper_module.DuelAcceptView(self.helperObj).decline.callback(click)
    ...
    async def test_cancel_challenge_from_the_challenger_deletes_the_pending_duel(self):
        click = self._click(555, 901)
        await helper_module.DuelAcceptView(self.helperObj).cancelChallenge.callback(click)

        self.cursor.execute("SELECT COUNT(*) FROM duels WHERE messageId=555")
        self.assertEqual(self.cursor.fetchone()[0], 0)
        # nothing was ever escrowed, so nothing to refund on a cancel
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "balance"), 1000)
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 902, "balance"), 1000)
        click.response.send_message.assert_awaited_once_with(
            "**Alice** cancelled the challenge against **Bob**."
        )
        click.message.edit.assert_awaited_once_with(view=None)

    async def test_cancel_challenge_from_the_target_is_rejected(self):
        click = self._click(555, 902)
        await helper_module.DuelAcceptView(self.helperObj).cancelChallenge.callback(click)
    ...
    async def test_cancel_challenge_ignores_mismatched_message_id(self):
        click = self._click(999, 901)
        await helper_module.DuelAcceptView(self.helperObj).cancelChallenge.callback(click)
    ...
    async def test_cancel_challenge_after_acceptance_is_already_gone(self):
        click_accept = self._click(555, 902)
        await helper_module.DuelAcceptView(self.helperObj).accept.callback(click_accept)

        click_cancel = self._click(555, 901)
        await helper_module.DuelAcceptView(self.helperObj).cancelChallenge.callback(click_cancel)

        click_cancel.response.send_message.assert_awaited_once_with(
            "This challenge is no longer pending.", ephemeral=True
        )
        # the now-AWAITING_RESULT duel is untouched
        self.cursor.execute("SELECT state FROM duels WHERE challengerId=901 AND targetId=902")
        self.assertEqual(self.cursor.fetchone()[0], "AWAITING_RESULT")
...
class DuelResultViewTests(HelperTestCase):
    ...
    async def test_result_click_rejects_a_non_admin_bystander(self):
        result_message_id = await self._accept()
        click = self._click(result_message_id, 999)
        click.user.guild_permissions.manage_guild = False
    ...
    async def test_result_click_allows_a_non_admin_participant(self):
        result_message_id = await self._accept()
        click = self._click(result_message_id, 902)  # the target, non-admin
        click.user.guild_permissions.manage_guild = False
    ...
    # Regression: an accepted duel used to have no way back at all -
    # only "Challenger Won"/"Target Won", with real gold already
    # escrowed on both sides. See DuelResultView's own comment.
    async def test_cancel_click_before_accept_is_rejected(self):
        click = self._click(555, 903)
        await helper_module.DuelResultView(self.helperObj).cancelDuel.callback(click)

        self.cursor.execute("SELECT state FROM duels WHERE messageId=555")
        self.assertEqual(self.cursor.fetchone()[0], "PENDING_ACCEPT")
        self.assertTrue(click.response.send_message.call_args.kwargs.get("ephemeral"))

    async def test_cancel_click_rejects_a_non_admin_bystander(self):
        result_message_id = await self._accept()
        click = self._click(result_message_id, 999)
        click.user.guild_permissions.manage_guild = False
    ...
    async def test_cancel_click_allows_a_non_admin_participant(self):
        result_message_id = await self._accept()
        click = self._click(result_message_id, 902)  # the target, non-admin
        click.user.guild_permissions.manage_guild = False
        ...
        self.assertIn("Cancel this duel?", click.response.send_message.call_args.args[0])

    async def test_cancel_click_posts_a_confirmation_instead_of_refunding(self):
        result_message_id = await self._accept()

        click = self._click(result_message_id, 903)
        await helper_module.DuelResultView(self.helperObj).cancelDuel.callback(click)

        # A real gold refund shouldn't hinge on the click alone either.
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "balance"), 750)
        view = click.response.send_message.call_args.kwargs["view"]
        self.assertIsInstance(view, helper_module.ConfirmDuelCancelView)
...
class ConfirmDuelResultViewTests(HelperTestCase):
    ...
    async def test_interaction_check_allows_a_participant(self):
        view = helper_module.ConfirmDuelResultView(self.helperObj, self.duel_id, True)
        click = self._click(user_id=902)  # the target, not the challenger
        click.user.guild_permissions.manage_guild = False

        self.assertTrue(await view.interaction_check(click))


class ConfirmDuelCancelViewTests(HelperTestCase):
    def setUp(self):
        super().setUp()
        self.channel = FakeChannel("general")
        self.helperObj.client = FakeClient(channels=[self.channel], guilds=[self.guild])
    ...
    async def test_confirm_refunds_both_players_exact_stake(self):
        view = helper_module.ConfirmDuelCancelView(self.helperObj, self.duel_id)
        await view.confirm.callback(self._click())

        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "balance"), 1000)
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 902, "balance"), 1000)
        # a cancellation isn't a loss/win for either side
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "wins"), 0)
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 901, "losses"), 0)
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 902, "wins"), 0)
        self.assertEqual(self.helperObj.getEconomy(GUILD_ID, 902, "losses"), 0)
    ...
    async def test_confirm_announces_the_refund_in_the_original_channel(self):
        view = helper_module.ConfirmDuelCancelView(self.helperObj, self.duel_id)
        await view.confirm.callback(self._click())

        self.channel.send.assert_awaited_once()
        message = self.channel.send.call_args.args[0]
        self.assertIn("Alice", message)
        self.assertIn("Bob", message)
        self.assertIn("cancelled", message)

    async def test_cancel_restores_the_duel_without_refunding(self):
        view = helper_module.ConfirmDuelCancelView(self.helperObj, self.duel_id)
        await view.cancel.callback(self._click())
...
class LeaderboardHelperTests(HelperTestCase):
    ...
    def _seed_players(self):
        ...
        # Cleo has no bets/games played yet, so her win rates should be None.
        self.cursor.execute(
    ...
    def test_get_leaderboard_entries_computes_rates_and_none_for_no_games(self):
        ...
        # entire game_wins/game_losses total should read as casual.
        # ranked_win_rate stays None the same way an empty game_win_rate
        # does.
    ...
    def test_filter_drops_everyone_from_a_ranked_view_when_nobody_has_played_ranked(self):
        # _seed_players deliberately gives nobody a ranked-tagged game
        # (see test_get_leaderboard_entries_computes_rates_and_none_for_no_games).
...
# Cards mode: /team lookup-style one-player-full-/stats-embed-per-page
...
# actual trading card (see LeaderboardPagingView). /leaderboard itself
# always posts the ranked list first. These tests drive the view's own
# Cards button (_handleLeaderboardViewCardsClick) to reach cards mode,
# the same way a real user would, rather than a cards= command param.
class LeaderboardCardsModeTests(HelperTestCase):
    ...
    # Posts the list (leaderboardHelper always starts there now) and
    # clicks its Cards button, returning the posted FakeMessage so
    # callers can inspect message.edit's own call args for the resulting
    # cards-mode render.
    async def _enter_cards_mode(self, ctx, stat, order, message_id):
        posted = FakeMessage(id=message_id)
        ctx.original_response.return_value = posted
        await self.helperObj.leaderboardHelper(ctx, stat, order)
    ...
    async def test_renders_the_first_ranked_players_stats_embed(self):
        ...
        posted = await self._enter_cards_mode(ctx, None, "desc", 7101)
    ...
    async def test_stores_the_cards_flag_alongside_the_rest_of_the_view_state(self):
        ...
        await self._enter_cards_mode(ctx, None, "desc", 7171)
    ...
    async def test_falls_back_gracefully_when_the_member_has_left(self):
        ...
        posted = await self._enter_cards_mode(ctx, None, "desc", 7103)
    ...
    async def test_cards_button_rejects_a_stale_or_gone_leaderboard(self):
        click = FakeInteraction(
            self.guild, FakeMember("Alice", id=901), channel=self.channel, message=FakeMessage(id=999)
        )
        await self.helperObj._handleLeaderboardViewCardsClick(click)
        self.assertTrue(click.response.send_message.call_args.kwargs.get("ephemeral"))

    async def test_list_mode_shows_the_cards_button_but_not_the_card_toggle(self):
        self._seed_players()
        ctx = self._ctx()
        ...
        view = ctx.response.send_message.call_args.kwargs["view"]
        self.assertIn(view.viewCards, view.children)
        self.assertNotIn(view.showCard, view.children)
        self.assertNotIn(view.returnToStats, view.children)
        self.assertNotIn(view.backToList, view.children)

    async def test_back_to_list_click_returns_to_the_ranked_list(self):
        self._seed_players()
        ctx = self._ctx()
        posted = await self._enter_cards_mode(ctx, None, "desc", 7104)
        posted.edit.reset_mock()

        click = FakeInteraction(self.guild, ctx.user, channel=self.channel, message=posted)
        await self.helperObj._handleLeaderboardBackToListClick(click)

        kwargs = posted.edit.call_args.kwargs
        self.assertIn("Alice", kwargs["embed"].description)
        view = kwargs["view"]
        self.assertIn(view.viewCards, view.children)
        self.assertNotIn(view.showCard, view.children)
        self.assertNotIn(view.backToList, view.children)
        self.cursor.execute("SELECT cards, cardShown, page FROM leaderboards WHERE messageId=7104")
        self.assertEqual(self.cursor.fetchone(), (0, 0, 0))
...
class TeamListPagingViewTests(HelperTestCase):
    ...
    async def test_preserves_member_filter_across_a_page_flip(self):
        ...
        # captain, so all 12 stay eligible under this filter. There's
        # still a real second page to flip to, proving the filter (not
        # just the footer text) carried over rather than resetting to
        # unfiltered.
        self.cursor.execute(
    ...
    async def test_jump_button_opens_a_modal_sized_to_the_real_page_count(self):
        view = helper_module.TeamListPagingView(self.helperObj)
        click = self._click()
        ...
        self.assertIn("1-2", modal.page_input.label)  # 12 teams / 10 per page = 2 pages
    ...
    async def test_modal_submit_jumps_straight_to_the_typed_page(self):
        click = self._click()
        modal = helper_module._PageJumpModal(self.helperObj, "_handleTeamListPageClick", 2)
...
class LeaderboardPagingViewTests(HelperTestCase):
    ...
    async def test_jump_button_opens_a_modal_sized_to_the_real_page_count(self):
        click = self._click()
        await helper_module.LeaderboardPagingView(self.helperObj).jump.callback(click)

        click.response.send_modal.assert_awaited_once()
        modal = click.response.send_modal.call_args.args[0]
        self.assertIsInstance(modal, helper_module._PageJumpModal)
        self.assertIn("1-3", modal.page_input.label)  # 25 entries / 10 per page = 3 pages

    async def test_jump_on_an_unknown_message_is_rejected(self):
        click = self._click(message=FakeMessage(id=424242))
        await helper_module.LeaderboardPagingView(self.helperObj).jump.callback(click)
        click.response.send_modal.assert_not_awaited()
        self.assertTrue(click.response.send_message.call_args.kwargs.get("ephemeral"))
    ...
    async def test_modal_submit_jumps_straight_to_the_typed_page(self):
        click = self._click()
        modal = helper_module._PageJumpModal(self.helperObj, "_handleLeaderboardPageClick", 3)
        modal.page_input._value = "3"

        await modal.on_submit(click)

        self.assertEqual(self._page(), 2)  # 1-based input, 0-based stored page
        embed = click.response.edit_message.call_args.kwargs["embed"]
        self.assertIn("Page 3/3", embed.footer.text)

    async def test_modal_submit_clamps_past_the_last_page(self):
        click = self._click()
        modal = helper_module._PageJumpModal(self.helperObj, "_handleLeaderboardPageClick", 3)
        ...
        self.assertEqual(self._page(), 2)  # clamped to the real last page

    async def test_modal_submit_rejects_non_numeric_input(self):
        click = self._click()
        modal = helper_module._PageJumpModal(self.helperObj, "_handleLeaderboardPageClick", 3)
    ...
    async def test_modal_submit_rejects_zero_and_negative_numbers(self):
        click = self._click()
        modal = helper_module._PageJumpModal(self.helperObj, "_handleLeaderboardPageClick", 3)
        modal.page_input._value = "0"
...
# loop, all sharing the one process-wide sqlite3 cursor (see
# helpers.__init__ in helper.py and its single instantiation in bot.py).
# There's no thread parallelism to worry about, but two guilds'
# coroutines genuinely can interleave at any `await` point. These tests
# run real asyncio.gather() calls across genuinely different guild_ids
# (never mocking the concurrency away) to prove each guild's state stays
# isolated no matter how the awaits land.
# ===========================================================================
...
class ConcurrentMultiGuildCommandsTests(HelperTestCase):
    ...
    async def test_guild_b_finishing_mid_flight_does_not_corrupt_guild_a(self):
        ...
        # for real. That guarantees guild B's entire _handleRosterStartClick
...
# project database and the real bot token, then exercise command
# callbacks and event handlers directly. Never connects to Discord.
# ===========================================================================
...
class BotModuleTestCase(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        # definitions now (see syncCommandsToGuild in bot.py), copied/
        # synced to a guild dynamically on_ready/on_guild_join rather than
        # one hardcoded testing guild id. So on_guild_join (called
        # throughout these tests via _insert_guild_row) would otherwise
        # try to hit the network. copy_global_to() is pure in-memory
        # bookkeeping and safe to leave real.
        ...
        # every single time the suite runs. This is saved before patching
        # so a test that specifically wants the real thing (see
    ...
    # been folded into an app_commands.Group. Group.commands holds its
    # own registered subcommands the same way the tree holds top-level
    # ones.
    ...
    # .voice = None by default, which would trip those guards. Defaults to
    # two members (not zero) so make-teams random's own "Not enough
    # players" guard doesn't trip every test that doesn't care about the
    # roster. Pass members= explicitly for a test that does.
    def _ctx_in_voice(self, guild_id=GUILD_ID, members=None):
        ctx = self._ctx(guild_id=guild_id)
        if members is None:
            members = [FakeMember("Voice1"), FakeMember("Voice2")]
        ctx.user.voice = FakeVoiceState(FakeChannel("Lobby", members=members))
        return ctx
...
class CommandRegistrationTests(BotModuleTestCase):
    # Regression: tree.sync() is mocked out in every BotModuleTestCase
    # test (see its own setUp comment: a real sync makes a network
    # call), so Discord's own server-side validation of command/param
    # name and description length (1-100 characters each) never actually
    # runs against these commands anywhere else in the suite. A
    # description that crept a few characters over 100 once made it all
    # the way to a real deploy before anything caught it, breaking
    # tree.sync() (and so every command in the same guild) outright. This
    # is a local stand-in for that same check.
    def test_every_command_and_param_name_and_description_fit_discords_limits(self):
        def walk(commands):
            for cmd in commands:
                if isinstance(cmd, app_commands.Group):
                    yield from walk(cmd.commands)
                else:
                    yield cmd

        for cmd in walk(self.bot.tree.get_commands()):
            self.assertTrue(
                1 <= len(cmd.name) <= 32, f"{cmd.qualified_name}: name is {len(cmd.name)} chars"
            )
            self.assertTrue(
                1 <= len(cmd.description) <= 100,
                f"{cmd.qualified_name}: description is {len(cmd.description)} chars",
            )
            for param in cmd.parameters:
                self.assertTrue(
                    1 <= len(param.name) <= 32,
                    f"{cmd.qualified_name} {param.name}: param name is {len(param.name)} chars",
                )
                self.assertTrue(
                    1 <= len(param.description) <= 100,
                    f"{cmd.qualified_name} {param.name}: param description is "
                    f"{len(param.description)} chars",
                )

    def test_all_expected_commands_registered(self):
        names = {c.name for c in self.bot.tree.get_commands()}
        expected = {
            "set", "wager", "daily", "give", "current-game",
            "stats", "card-set", "shop",
            "achievements",
    ...
    def test_wager_group_has_the_expected_subcommands(self):
        names = {c.name for c in self._command("wager").commands}
        self.assertEqual(names, {"team", "against"})

    def test_shop_group_has_the_expected_subcommands(self):
        names = {c.name for c in self._command("shop").commands}
        self.assertEqual(names, {"preview", "browse", "buy"})
    ...
    def test_tournament_group_has_the_expected_subcommands(self):
        ...
        self.assertEqual(
            names, {"create", "register", "unregister", "create-bracket", "print-bracket", "start"}
        )
...
class MyTeamAutocompleteTests(BotModuleTestCase):
    ...
    # Same Manage Server carve-out as myCaptainedTeamAutocomplete. An
    # admin sees every team in the guild here too, not just ones they're
    # rostered on.
...
class TeamStatsAutocompleteTests(BotModuleTestCase):
    async def test_empty_input_behaves_like_my_team_autocomplete(self):
        ctx = self._ctx(manage_guild=False)
        with patch.object(
            self.bot.helperObj, "getTeamsForPlayer", return_value=_named_teams("Red Dragons")
        ) as player_mock, patch.object(
            self.bot.helperObj, "getTeamsForGuild", return_value=_named_teams("Red Dragons", "Blue Sharks")
        ) as guild_mock:
            choices = await self.bot.teamStatsAutocomplete(ctx, "")
        self.assertEqual([c.value for c in choices], ["Red Dragons"])
        player_mock.assert_called_once()
        guild_mock.assert_not_called()

    # The whole point of this override: once the caller actually types
    # something, that's a deliberate search. It widens to every team in
    # the guild instead of staying scoped to just the caller's own.
    async def test_non_empty_input_searches_every_team_in_the_guild(self):
        ctx = self._ctx(manage_guild=False)
        with patch.object(
            self.bot.helperObj, "getTeamsForPlayer", return_value=[]
        ) as player_mock, patch.object(
            ...
            choices = await self.bot.teamStatsAutocomplete(ctx, "sha")
        self.assertEqual([c.value for c in choices], ["Blue Sharks"])
        guild_mock.assert_called_once_with(ctx.guild.id)
        player_mock.assert_not_called()


class WagerTeamAutocompleteTests(BotModuleTestCase):
    async def test_suggests_the_current_games_real_team_names(self):
        guild_id = 9320
        ...
        choices = await self.bot.wagerTeamAutocomplete(ctx, "")

        self.assertEqual([(c.name, c.value) for c in choices], [("Ravens", "1"), ("Falcons", "2")])

    async def test_filters_by_current_input(self):
        guild_id = 9321
        await self._insert_guild_row(guild_id)
        ctx = self._ctx(guild_id=guild_id)
        team1, team2 = Team(), Team()
        ...
        choices = await self.bot.wagerTeamAutocomplete(ctx, "fal")

        self.assertEqual([c.name for c in choices], ["Falcons"])

    async def test_falls_back_to_generic_labels_for_an_unnamed_roster(self):
        guild_id = 9322
        await self._insert_guild_row(guild_id)
        ctx = self._ctx(guild_id=guild_id)
        ...
        self.assertEqual([c.name for c in choices], ["Team 1", "Team 2"])
...
class GuildLifecycleEventTests(BotModuleTestCase):
    ...
    async def test_on_ready_registers_persistent_views_exactly_once(self):
        # Persistent views (fixed custom_id, timeout=None) need
        # registering once per process so their buttons keep routing to
        # this bot across a restart. on_ready can fire more than once
        # (e.g. on reconnect), so the second call here must not
        # re-register.
        with patch.object(self.bot.client, "add_view") as mock_add_view, \
             patch.object(self.bot.client, "add_dynamic_items") as mock_add_dynamic_items:
            await self.bot.on_ready()
            first_call_count = mock_add_view.call_count
            first_dynamic_call_count = mock_add_dynamic_items.call_count
            await self.bot.on_ready()

        self.assertEqual(mock_add_view.call_count, first_call_count)
        self.assertEqual(mock_add_dynamic_items.call_count, first_dynamic_call_count)
        mock_add_dynamic_items.assert_any_call(self.bot.helper._DraftPickSlotButton)
...
class LoggingCommandTreeTests(BotModuleTestCase):
    ...
    async def test_survives_an_exception_from_real_discord_py_resolution(self):
        ...
        # on_app_command_error or the log at all) instead of just
        # skipping this one log line and letting the real command still
        # run.
    ...
    async def test_stale_bytecode_not_callable_typeerror_is_silently_skipped(self):
        # "'NoneType' object is not callable" specifically (not any other
        # exception) is the signature left behind when this process is
        # still running on bytecode compiled before a live edit to bot.py.
        # See the comment on the except TypeError branch just above this
        # method in bot.py. Nothing else in this code path should be able
        # to produce that exact message, so it's deliberately not logged,
        # unlike every other exception here.
        class _RaisingInteraction:
            type = discord.InteractionType.application_command
            data = {"name": "team-create"}
            user = FakeMember("Alice", id=901)
    ...
    async def test_other_typeerrors_are_still_logged(self):
        class _RaisingInteraction:
            type = discord.InteractionType.application_command
            data = {"name": "team-create"}
            user = FakeMember("Alice", id=901)
...
# helper.SHOCKWAVE_DEVELOPER_ID lives on the helper module, which (unlike
# bot, see _import_bot_module) is never popped from sys.modules between
# tests, so it's the same object every test in the whole process shares.
# Every test class below that touches it saves/restores the original
# value, so a mutation here can't leak into an unrelated test (e.g. the
# CardTitleTests "Developer" title tests) that happens to run afterward.
class DeveloperAlertsDMCommandTests(BotModuleTestCase):
    def setUp(self):
        super().setUp()
        self._original_developer_id = self.bot.helper.SHOCKWAVE_DEVELOPER_ID
        self.bot.helper.SHOCKWAVE_DEVELOPER_ID = 999
        self.addCleanup(setattr, self.bot.helper, "SHOCKWAVE_DEVELOPER_ID", self._original_developer_id)

    def _dm(self, content, author_id=999):
    ...
    async def test_alerts_off(self):
        await self.bot.on_message(self._dm("alerts off"))
        self.assertFalse(self.bot._developerAlertsEnabled)

    async def test_alerts_on(self):
        self.bot._developerAlertsEnabled = False
        await self.bot.on_message(self._dm("alerts on"))
        self.assertTrue(self.bot._developerAlertsEnabled)

    async def test_reply_reflects_the_new_state(self):
        message = self._dm("alerts off")
        await self.bot.on_message(message)
        message.channel.send.assert_awaited_once()
        self.assertIn("off", message.channel.send.call_args.args[0])

    async def test_alerts_status_reports_without_changing_anything(self):
        self.bot._developerAlertsEnabled = False
        message = self._dm("alerts status")
        await self.bot.on_message(message)
        self.assertFalse(self.bot._developerAlertsEnabled)
        self.assertIn("off", message.channel.send.call_args.args[0])
    ...
    async def test_ignores_unrecognized_dm_text(self):
        message = self._dm("hello there")
        await self.bot.on_message(message)
        message.channel.send.assert_not_awaited()
        self.assertTrue(self.bot._developerAlertsEnabled)

    async def test_ignores_a_dm_from_anyone_else(self):
        message = self._dm("alerts off", author_id=1)
        await self.bot.on_message(message)
        message.channel.send.assert_not_awaited()
        self.assertTrue(self.bot._developerAlertsEnabled)

    async def test_ignores_the_same_text_sent_in_a_guild_channel(self):
        message = self._dm("alerts off")
    ...
    async def test_ignores_everything_when_no_developer_id_is_configured(self):
        self.bot.helper.SHOCKWAVE_DEVELOPER_ID = None
        message = self._dm("alerts off")
        await self.bot.on_message(message)
        message.channel.send.assert_not_awaited()
        self.assertTrue(self.bot._developerAlertsEnabled)
...
class DeveloperDMHandlerTests(BotModuleTestCase):
    def setUp(self):
        super().setUp()
        self._original_developer_id = self.bot.helper.SHOCKWAVE_DEVELOPER_ID
        self.bot.helper.SHOCKWAVE_DEVELOPER_ID = 999
    ...
    def _record(self, level=logging.WARNING, msg="uh oh"):
        return logging.LogRecord("shockwave", level, __file__, 1, msg, None, None)

    def test_emit_does_nothing_when_alerts_are_disabled(self):
        self.bot._developerAlertsEnabled = False
    ...
    def test_emit_does_nothing_when_no_developer_id_is_configured(self):
        self.bot.helper.SHOCKWAVE_DEVELOPER_ID = None
        with patch.object(self.bot.client, "is_ready", return_value=True), \
             patch.object(self.bot.asyncio, "run_coroutine_threadsafe") as mock_schedule:
            self.bot._developer_dm_handler.emit(self._record())
    ...
    def test_emit_ignores_info_level_records(self):
        ...
        # out. Nothing about the record's own logger name/level does that
        # on its own.
        with patch.object(self.bot.client, "is_ready", return_value=True), \
             patch.object(self.bot.asyncio, "run_coroutine_threadsafe") as mock_schedule:
            self.bot.logger.handle(self._record(level=logging.INFO, msg="just routine"))
        mock_schedule.assert_not_called()

    def test_emit_queues_the_message_when_the_client_is_not_ready(self):
        with patch.object(self.bot.client, "is_ready", return_value=False):
            self.bot._developer_dm_handler.emit(self._record(msg="startup trouble"))
        self.assertEqual(len(self.bot._pendingDeveloperDMs), 1)
        self.assertIn("startup trouble", self.bot._pendingDeveloperDMs[0])

    def test_emit_schedules_a_dm_when_the_client_is_ready(self):
        with patch.object(self.bot.client, "is_ready", return_value=True), \
             patch.object(self.bot.asyncio, "run_coroutine_threadsafe") as mock_schedule:
            self.bot._developer_dm_handler.emit(self._record(msg="ready trouble"))
        mock_schedule.assert_called_once()
        coro, loop = mock_schedule.call_args.args
        self.assertEqual(loop, self.bot.client.loop)
        coro.close()  # never actually scheduled onto a real loop; avoids a "never awaited" warning
        self.assertEqual(self.bot._pendingDeveloperDMs, [])

    async def test_send_developer_dm_fetches_and_sends(self):
        fake_user = SimpleNamespace(send=AsyncMock())
        with patch.object(self.bot.client, "fetch_user", AsyncMock(return_value=fake_user)) as mock_fetch:
            await self.bot._sendDeveloperDM("hello developer")
        mock_fetch.assert_awaited_once_with(999)
        fake_user.send.assert_awaited_once_with("hello developer")

    async def test_send_developer_dm_truncates_overly_long_messages(self):
        fake_user = SimpleNamespace(send=AsyncMock())
        with patch.object(self.bot.client, "fetch_user", AsyncMock(return_value=fake_user)):
            await self.bot._sendDeveloperDM("x" * 3000)
        sent = fake_user.send.call_args.args[0]
        self.assertLessEqual(len(sent), 2000)
        self.assertTrue(sent.endswith("truncated, see shockwave.log)"))

    async def test_send_developer_dm_swallows_failures_without_logging(self):
        with patch.object(self.bot.client, "fetch_user", AsyncMock(side_effect=RuntimeError("no connection"))):
            with self.assertNoLogs(self.bot.logger, level="WARNING"):
                await self.bot._sendDeveloperDM("hello")

    async def test_flush_pending_sends_each_queued_message_in_order_and_clears_the_queue(self):
        self.bot._pendingDeveloperDMs.extend(["first", "second"])
        fake_user = SimpleNamespace(send=AsyncMock())
        with patch.object(self.bot.client, "fetch_user", AsyncMock(return_value=fake_user)):
            await self.bot._flushPendingDeveloperDMs()
        sent_messages = [call.args[0] for call in fake_user.send.call_args_list]
        self.assertEqual(sent_messages, ["first", "second"])
        self.assertEqual(self.bot._pendingDeveloperDMs, [])

    async def test_flush_pending_is_a_no_op_when_the_queue_is_empty(self):
        with patch.object(self.bot.client, "fetch_user", AsyncMock()) as mock_fetch:
            await self.bot._flushPendingDeveloperDMs()
...
class AppCommandCompletionTests(BotModuleTestCase):
    ...
    async def test_survives_a_logging_failure(self):
        ...
        # interaction the way interaction_check's own could. It's still
        # caught explicitly so it reaches this file's own log instead of
        # only discord.py's default stderr-only on_error.
...
class LogDatabaseStatementTests(BotModuleTestCase):
    ...
    async def test_a_real_mutation_reaches_the_log_via_the_trace_callback(self):
        ...
        # bot.py) reaches the log. Not just a direct call to the
...
class BackupDatabaseTests(BotModuleTestCase):
    def setUp(self):
        super().setUp()
        # The real _backupDatabase (see self._real_backupDatabase, saved
        # by the parent setUp before _backupDatabase itself gets mocked
        # out) writes wherever BACKUP_DIR points. That's redirected to a
        # throwaway temp directory here so this class's own real-backup
        # assertions never touch data/guildData/backups/.
...
class ExpireInvitesTaskTests(BotModuleTestCase):
    async def test_task_calls_the_real_expiry_and_logs_when_something_expired(self):
        with patch.object(
            self.bot.helperObj, "expireStalePendingInvites", MagicMock(return_value=(2, 1, 1))
        ) as mock_expire:
            with self.assertLogs(self.bot.logger, level="INFO") as cm:
                await self.bot.expireInvitesTask.coro()
        mock_expire.assert_called_once()
        self.assertIn("Expired", cm.output[0])

    async def test_task_stays_quiet_when_nothing_expired(self):
        with patch.object(
            self.bot.helperObj, "expireStalePendingInvites", MagicMock(return_value=(0, 0, 0))
        ):
            # assertNoLogs isn't available on this Python version's
            # unittest, so this just confirms the call completes cleanly.
            # The "if invites_expired or duels_expired or transfers_expired"
            # guard itself is what keeps this quiet in practice.
            await self.bot.expireInvitesTask.coro()

    async def test_task_logs_and_swallows_a_failure(self):
        with patch.object(
            self.bot.helperObj, "expireStalePendingInvites", MagicMock(side_effect=Exception("db locked"))
        ):
            with self.assertLogs(self.bot.logger, level="ERROR") as cm:
                await self.bot.expireInvitesTask.coro()  # must not raise
        self.assertIn("Expiring stale invites/challenges failed", cm.output[0])
...
class RunStartupSelfTestsTests(BotModuleTestCase):
    ...
    # and a little too recursive for comfort). They do this by patching
    # subprocess.run to write a hand-built junitxml report to whatever
    # path the real `--junitxml=<path>` argument names, and return a fake
    # CompletedProcess. This exercises _runStartupSelfTests' own
    # parsing/logging logic exactly as it runs against a genuine
    # pytest-produced report.
    ...
    def test_missing_report_logs_a_warning_without_crashing(self):
        # pytest ran (or tried to) but never wrote a report at all: e.g.
        # a collection error, or pytest-xdist missing from a stale
        # install.
...
class GlobalErrorHandlerTests(BotModuleTestCase):
    def _ctx_with_response_done(self, done):
        ...
        # explicitly here rather than just awaited like everything else
        # on it.
    ...
    async def test_missing_permissions_does_not_log_a_variable_dump(self):
        # Only the "something went wrong" branch needs the extra
        # diagnostic context. MissingPermissions already gets a specific,
        # expected user-facing message and isn't a bug to dump state for.
        ctx = self._ctx_with_response_done(False)
        error = app_commands.MissingPermissions(["manage_guild"])
...
class CommandDelegationTests(BotModuleTestCase):
    async def test_wager_passes_resolved_team_value(self):
        ...
        with patch.object(self.bot.helperObj, "wagerHelper", mock):
            await self._command("wager team").callback(ctx, 250, "2")
        mock.assert_awaited_once_with(ctx, 250, 2, None)

    async def test_wager_resolves_the_teams_real_name_too(self):
        guild_id = 9310
        await self._insert_guild_row(guild_id)
        ctx = self._ctx(guild_id=guild_id)
        team1 = Team()
        ...
        mock = AsyncMock()
        with patch.object(self.bot.helperObj, "wagerHelper", mock):
            await self._command("wager team").callback(ctx, 250, "falcons")
        mock.assert_awaited_once_with(ctx, 250, 2, None)

    async def test_wager_rejects_an_unrecognized_team(self):
        guild_id = 9311
        await self._insert_guild_row(guild_id)
        ctx = self._ctx(guild_id=guild_id)
        mock = AsyncMock()
        with patch.object(self.bot.helperObj, "wagerHelper", mock):
            await self._command("wager team").callback(ctx, 250, "Not A Team")
        mock.assert_not_awaited()
        args, kwargs = ctx.response.send_message.call_args
        self.assertIn("Not A Team", args[0])
        self.assertTrue(kwargs.get("ephemeral"))
    ...
    async def test_give_delegates(self):
        ctx = self._ctx()
        target = FakeMember("Bob", id=902)
        mock = AsyncMock()
        with patch.object(self.bot.helperObj, "giveGoldHelper", mock):
            await self._command("give").callback(ctx, target, 100)
        mock.assert_awaited_once_with(ctx, target, 100)

    async def test_current_game_delegates(self):
        ctx = self._ctx()
        mock = AsyncMock()
        with patch.object(self.bot.helperObj, "currentGameHelper", mock):
            await self._command("current-game").callback(ctx)
        mock.assert_awaited_once_with(ctx)
    ...
    async def test_unregister_team_delegates(self):
        ctx = self._ctx()
        mock = AsyncMock()
        with patch.object(self.bot.helperObj, "unregisterTeamHelper", mock):
            await self._command("tournament unregister").callback(ctx, "Red")
        mock.assert_awaited_once_with(ctx, "Red")
    ...
    async def test_team_remove_delegates(self):
        ctx = self._ctx()
        target = FakeMember("Bob", id=902)
        mock = AsyncMock()
        with patch.object(self.bot.helperObj, "teamRemoveHelper", mock):
            await self._command("team remove").callback(ctx, "Red", target)
    ...
    async def test_preview_delegates_with_resolved_type_value(self):
        ...
        with patch.object(self.bot.helperObj, "previewHelper", mock):
            await self._command("shop preview").callback(ctx, type=choice)
    ...
    async def test_shop_delegates(self):
        ...
        with patch.object(self.bot.helperObj, "shopHelper", mock):
            await self._command("shop browse").callback(ctx)
        mock.assert_awaited_once_with(ctx)
...
class HelpCommandTests(BotModuleTestCase):
    ...
    async def test_no_command_points_new_players_at_setup(self):
        ctx = self._ctx()
        await self._command("help").callback(ctx, command=None)
        message = ctx.response.send_message.call_args.args[0]
        self.assertIn("/setup", message)
    ...
    async def test_known_command_describes_it_with_usage(self):
        ctx = self._ctx()
        await self._command("help").callback(ctx, command="wager team")
        message = ctx.response.send_message.call_args.args[0]
        self.assertIn("/wager team", message)
        self.assertIn("<amount>", message)
        self.assertIn("<team>", message)
        self.assertIn(self.bot.COMMAND_HELP["wager team"], message)
    ...
    def test_every_registered_command_has_an_entry(self):
        ...
        # against tree.get_commands() directly. It collects each leaf's
        # qualified_name (the same "group sub" shape COMMAND_HELP is
        # keyed by, e.g. "make-teams random").
        names = set()
...
class ClearCommandTests(BotModuleTestCase):
    ...
    async def test_clear_channels_does_not_wipe_channel_names_until_confirmed(self):
        guild_id = 903
        await self._insert_guild_row(guild_id)
        ...
        view = ctx.response.send_message.call_args.kwargs["view"]
        self.assertIsInstance(view, helper_module.ConfirmClearActionView)

        click = self._ctx(guild_id=guild_id)
        await view.confirm.callback(click)

        self.assertEqual(self.bot.helperObj.get(guild_id, "channel1"), "")
        self.assertIn("channel names", click.response.edit_message.call_args.kwargs["content"])

    async def test_clear_tournament_does_not_wipe_match_history_until_confirmed(self):
        guild_id = 9037
        await self._insert_guild_row(guild_id)
        ...
        # Regression: clear_tournament used to write to a dead
        # servers.tournament column and never actually touch a real
        # /tournament create tournament.
        self.assertIsNotNone(self.bot.helperObj.getTournament(guild_id))
        view = ctx.response.send_message.call_args.kwargs["view"]
    ...
    async def test_clear_teams_does_not_wipe_the_draft_until_confirmed(self):
        guild_id = 9033
        await self._insert_guild_row(guild_id)
        self.bot.helperObj.update(guild_id, "mode", "Draft")
        ctx = self._ctx(guild_id=guild_id)

        await self._command("clear teams").callback(ctx)

        self.assertEqual(self.bot.helperObj.get(guild_id, "mode"), "Draft")
        view = ctx.response.send_message.call_args.kwargs["view"]
        self.assertIsInstance(view, helper_module.ConfirmClearActionView)
        ...
        self.assertEqual(self.bot.helperObj.get(guild_id, "mode"), "Normal")
        click.response.edit_message.assert_awaited_once_with(content="Cleared!", view=view)

    async def test_cancelling_clear_teams_leaves_the_draft_untouched(self):
        guild_id = 9039
        await self._insert_guild_row(guild_id)
        self.bot.helperObj.update(guild_id, "mode", "Draft")
        ctx = self._ctx(guild_id=guild_id)
        ...
        click.response.edit_message.assert_awaited_once_with(
            content="Cancelled. Nothing was cleared.", view=view
        )

    async def test_clear_action_view_rejects_non_invoker(self):
        guild_id = 9040
        await self._insert_guild_row(guild_id)
        ctx = self._ctx(guild_id=guild_id)
    ...
    async def test_clear_action_view_timeout_marks_the_prompt_expired(self):
        guild_id = 9041
        await self._insert_guild_row(guild_id)
        self.bot.helperObj.update(guild_id, "mode", "Draft")
        ctx = self._ctx(guild_id=guild_id)
        ...
        self.assertIn("expired", view.message.edit.call_args.kwargs["content"])
        self.assertTrue(view.confirm.disabled)
    ...
    async def test_clear_economy_does_not_wipe_stats_until_confirmed(self):
        ...
        # the guild-wide economy wipe waits on the confirmation view.
    ...
    # Regression: /clear elo and /clear economy used to have no way to
    # target a single player at all, unlike their siblings /clear
    # achievements and /clear card-unlocks.
    async def test_clear_elo_with_a_user_only_resets_that_players_elo(self):
        guild_id = 9046
        await self._insert_guild_row(guild_id)
        self.bot.helperObj.ensureEconomyRow(guild_id, 901, "Alice")
        self.bot.helperObj.ensureEconomyRow(guild_id, 902, "Bob")
        self.bot.helperObj.ensureGameStatsRow(guild_id, 901, "Alice", "League")
        self.bot.helperObj.ensureGameStatsRow(guild_id, 902, "Bob", "League")
        self.bot.cursor.execute(
            "UPDATE game_stats SET elo=1500 WHERE guildId=? AND game='League'", (guild_id,)
        )
        self.bot.mainDB.commit()
        target = FakeMember("Alice", id=901)
        ctx = self._ctx(guild_id=guild_id)

        await self._command("clear elo").callback(ctx, user=target)
        ...
        # Bob's elo survives untouched.
        self.assertEqual(self.bot.helperObj.getGameStat(guild_id, 902, "League", "elo"), 1500)
        self.assertIn(target.mention, click.response.edit_message.call_args.kwargs["content"])

    async def test_clear_economy_with_a_user_only_resets_that_players_economy(self):
        guild_id = 9047
        await self._insert_guild_row(guild_id)
        self.bot.helperObj.ensureEconomyRow(guild_id, 901, "Alice")
        self.bot.helperObj.ensureEconomyRow(guild_id, 902, "Bob")
        self.bot.cursor.execute(
            "UPDATE economy SET balance=1000 WHERE guildId=?", (guild_id,)
        )
        self.bot.mainDB.commit()
        target = FakeMember("Alice", id=901)
        ctx = self._ctx(guild_id=guild_id)

        await self._command("clear economy").callback(ctx, user=target)
        ...
        self.bot.cursor.execute(
            "SELECT COUNT(*) FROM economy WHERE guildId=? AND userId=?", (guild_id, 901)
        )
        self.assertEqual(self.bot.cursor.fetchone()[0], 0)
        # Bob's economy row survives untouched.
        self.assertEqual(self.bot.helperObj.getEconomy(guild_id, 902, "balance"), 1000)
        self.assertIn(target.mention, click.response.edit_message.call_args.kwargs["content"])
    ...
    # Regression: /clear elo (and economy/achievements/card-unlocks) used
    # to clear the current teams/draft immediately, before this prompt
    # even posted, so Cancel's "Nothing was reset" was a lie whenever a
    # draft was in progress. It's gated behind confirmation now too, same
    # as /clear teams itself (see ConfirmResetView.confirm).
    async def test_clear_elo_does_not_wipe_the_draft_until_confirmed(self):
        guild_id = 9042
        await self._insert_guild_row(guild_id)
        self.bot.helperObj.update(guild_id, "mode", "Draft")
        ctx = self._ctx(guild_id=guild_id)
    ...
    async def test_cancelling_clear_elo_leaves_the_draft_untouched(self):
        guild_id = 9045
        await self._insert_guild_row(guild_id)
        self.bot.helperObj.update(guild_id, "mode", "Draft")
        ctx = self._ctx(guild_id=guild_id)
...
class NotifyCommandTests(BotModuleTestCase):
    ...
    # Regression: an empty role used to fall through to the same success
    # branch a real send does (0 sent, 0 failures), reporting "Sent an
    # invite to 0 members in Squad!", a false success for nothing that
    # was actually sent.
    async def test_notify_role_with_no_members_reports_nobody_to_invite(self):
        guild_id = 919
        await self._insert_guild_row(guild_id)
        ctx = self._ctx_in_voice(guild_id=guild_id)
        role = SimpleNamespace(name="Squad", members=[])
        ...
        mock.assert_not_awaited()
        ctx.response.send_message.assert_awaited_once_with("Nobody's in Squad, so there's nobody to invite.")

    async def test_notify_member_reports_closed_dms(self):
        guild_id = 910
        await self._insert_guild_row(guild_id)
        ctx = self._ctx_in_voice(guild_id=guild_id)
        target = FakeMember("Target")
    ...
    async def test_notify_role_reports_partial_dm_failures(self):
        guild_id = 911
        await self._insert_guild_row(guild_id)
        ctx = self._ctx_in_voice(guild_id=guild_id)
        role = SimpleNamespace(name="Squad", members=[FakeMember("A"), FakeMember("B"), FakeMember("C")])

        mock = AsyncMock(side_effect=[True, False, True])
        with patch.object(self.bot.helperObj, "notifyHelper", mock):
            await self._command("notify").callback(ctx, role=role)

        ctx.response.send_message.assert_awaited_once_with(
            "Sent an invite to 2/3 members in Squad; 1 couldn't be DMed."
        )
...
class MakeTeamsCommandTests(BotModuleTestCase):
    ...
    # Regression: unlike /make-teams draft, random used to have no
    # "not enough players" guard at all. A lone caller got "Teams
    # created!" with one whole side left empty instead of this message.
    async def test_rejects_when_not_enough_players_in_voice_channel(self):
        ctx = self._ctx_in_voice(members=[FakeMember("Solo")])
        mock = AsyncMock()
        with patch.object(self.bot.helperObj, "randomizeTeamHelper", mock):
            await self._command("make-teams random").callback(ctx, use_roles=False)
        mock.assert_not_awaited()
        ctx.response.send_message.assert_awaited_once_with("Not enough players in the voice channel!")
    ...
    async def test_use_roles_forwards_the_flag_to_print_embed_and_finalize(self):
        ...
        randomize_mock.assert_awaited_once_with(ctx)
        # Regression test: /make-teams use_roles:True used to never
        # forward that flag to printEmbed, so roles never actually showed
        # up.
        embed_mock.assert_awaited_once()
    ...
    async def test_use_roles_explains_when_a_team_is_not_five(self):
        ...
        # This still continues normally (teams still get created and
        # posted) but says why no roles showed up.
        guild_id = 912
        await self._setup_teams(guild_id)
        ctx = self._ctx_in_voice(guild_id=guild_id)
    ...
    async def test_use_roles_allows_when_everyone_in_voice_has_run_setup(self):
        ...
        await self._setup_teams(guild_id)
        ctx = self._ctx_in_voice(guild_id=guild_id)
        ctx.user.voice.channel.members = [FakeMember("Ready", id=952), FakeMember("AlsoReady", id=955)]
    ...
    async def test_use_roles_ignores_bots_that_have_not_run_setup(self):
        ...
        await self._setup_teams(guild_id)
        ctx = self._ctx_in_voice(guild_id=guild_id)
        ctx.user.voice.channel.members = [FakeMember("SomeBot", id=953, bot=True), FakeMember("Human", id=956)]
    ...
    async def test_use_roles_false_never_checks_setup(self):
        ...
        await self._setup_teams(guild_id)
        ctx = self._ctx_in_voice(guild_id=guild_id)
        ctx.user.voice.channel.members = [FakeMember("NotReady", id=954), FakeMember("AlsoNotReady", id=957)]
...
class RankedCommandTests(BotModuleTestCase):
    ...
    async def test_ranked_use_roles_proceeds_and_notes_who_has_not_run_setup(self):
        ...
        ctx.user.voice.channel.members = [not_setup_member, FakeMember("AlsoNotReady", id=961)]
...
# backupDatabaseTask's daily snapshots. It's never imported by
# bot.py/helper.py, so these tests exercise it directly, always against a
# throwaway temp directory (DB_PATH/BACKUP_DIR patched for the duration
# of each test), never the real data/guildData/ paths it points at by
# default.
# ===========================================================================
...
class RestoreBackupTests(unittest.TestCase):
    ...
    def test_main_with_no_backups_present_returns_without_raising(self):
        # No main.db yet either. This is the very first run on a fresh
        # install, before backupDatabaseTask has ever produced a snapshot.
        restore_backup.main()
